...
bool VissimNet::Link_Fields (void)
{
	...
	Dir_Data *dir_in_ptr, *dir_out_ptr, *dir_ptr;
	...
	if (pair_itr->first.Equals ("<link")) {
		...
		key_itr = keep_list.find (link_data.Link ());

		connect_flag = (ab_key.first >= 0 && ab_key.second >= 0 && key_itr != keep_list.end ());
		dir = 0;
		dir_ptr = 0;

		//---- regular links ----

		if ((ab_key.first < 0 && ab_key.second < 0) || connect_flag) {
		...
		if ((ab_key.first >= 0 && ab_key.second >= 0) || connect_flag) {
			...
			if (connect_flag) {
				...
				if (map_stat.second) {
					...
					dir_ptr->First_Connect_From (index);

					key_itr->second.first = index;
				} else {
					...
					key_itr->second.first = map_stat.first->second;
				}

				connect_data.Dir_Index (dir);
				connect_data.To_Index (to_ptr->AB_Dir ());
				connect_data.Node (-1);
				connect_data.Low_Lane (0);
				connect_data.High_Lane (dir_data.Lanes () - 1);
				connect_data.To_Low_Lane (MAX ((to_lane - dir_data.Lanes () + 1), 0));
				connect_data.To_High_Lane (to_lane);
				connect_data.Type (THRU);

				connect_data.Next_To (dir_ptr->First_Connect_To ());
				...
				if (map_stat.second) {
					...
					dir_ptr->First_Connect_To (index);
					dir_out_ptr->First_Connect_From (index);

					key_itr->second.second = index;
				} else {
					connect_ptr = &connect_array [map_stat.first->second];
					...
					key_itr->second.second = map_stat.first->second;
				}
				...
				if (map_stat.second) {
					...
					connect_link.insert (Int_Map_Data (link_data.Link (), index));
				} else {
					connect_ptr = &connect_array [map_stat.first->second];
					...
					connect_link.insert (Int_Map_Data (link_data.Link (), map_stat.first->second));
				}
			}
		}
	}
