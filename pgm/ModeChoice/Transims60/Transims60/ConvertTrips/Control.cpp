...
void ConvertTrips::Program_Control (void)
{
	...
	random_duration.Seed (random.Seed () + 5);
	random_return.Seed (random.Seed () + 6);
	...
	for (i=1; i <= num_file; i++) {
		...
		if (!key.empty ()) {
			...
			//---- get the trip table name ----

			key = Get_Control_Text (TRIP_TABLE_NAME, i);

			if (key.empty ()) {
				group->Table (0);
			} else {
				group->Table (group->Trip_File ()->Required_Field (key));

				Print (0, String (" (Table=%d)") % (group->Table () + 1));
			}
		...
		group->Function (Get_Control_Integer (ACTIVITY_DURATION_FUNCTION, i));

		if (group->Function () > 0 && !functions.Function_Check (group->Function ())) {
			Error (String ("Activity Duration Function % is Not Defined") % group->Function ());
		}
		...
		if (group->Return_Flag ()) {

			if (group->Duration () == 0 && group->Function () == 0) {
				Error ("Activity Duration or Function is Required for Return Trips");
			}

			//---- get the percent return trips ----

			group->Return_Percent (Get_Control_Integer (PERCENT_RETURN_TRIPS, i));

			//---- get return purpose ----

			group->Return_Purpose (Get_Control_Integer (RETURN_TRIP_PURPOSE, i));

			//---- get return mode ----

			key = Get_Control_Text (RETURN_TRIP_MODE, i);

			if (!key.empty ()) {
				group->Return_Mode (Mode_Code (key));
				if (key [0] >= '0' && key [0] <= '9') {
					Print (0, String (" (%s)") % Mode_Code ((Mode_Type) group->Return_Mode ()));
				}
			}
		}
