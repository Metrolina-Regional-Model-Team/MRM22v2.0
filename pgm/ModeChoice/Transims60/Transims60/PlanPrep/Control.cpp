...
void PlanPrep::Program_Control (void)
{
	...
	if (!skim_flag) {
		...
		if (!key.empty ()) {
			...
			merge_mode_flag = Get_Control_Flag (MERGE_MODE_FLAG);
		}
	...
	//---- constraint parking lots ----

	key = Get_Control_Text (CONSTRAIN_PARKRIDE_LOTS);

	if (!key.empty () && !key.Equals ("NONE")) {
		park_list_flag = true;
		if (!park_list.Add_Ranges (key)) {
			Error ("Adding ParkRide Lot Ranges");
		}
	}

	max_ratio = Get_Control_Double (MAX_PARKING_DEMAND_RATIO);

	//---- constraint transit loads ----

	key = Get_Control_Text (CONSTRAIN_TRANSIT_LOADS);

	if (!key.empty () && !key.Equals ("NONE")) {
		route_list_flag = true;
		if (!route_list.Add_Ranges (key)) {
			Error ("Adding Transit Route Ranges");
		}
	}
	max_factor = Get_Control_Double (MAX_ROUTE_LOAD_FACTOR);
