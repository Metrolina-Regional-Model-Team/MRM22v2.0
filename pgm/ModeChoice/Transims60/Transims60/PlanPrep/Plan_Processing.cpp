...
{
	int part = 0;
	random_part.Seed (exe->random.Seed () + 1000 * (part + 1));
	...
	if (exe->plan_skim_flag) {
		plan_skim_file = new Plan_Skim_File (exe->plan_skim_file->File_Access (), exe->plan_skim_file->Dbase_Format ());
		plan_skim_file->Part_Flag (exe->plan_skim_file->Part_Flag ());
		plan_skim_file->Pathname (exe->plan_skim_file->Pathname ());
		plan_skim_file->First_Open (false);
	}
	...
	exe->num_new_skims += num_new_skims;
	END_LOCK
}
