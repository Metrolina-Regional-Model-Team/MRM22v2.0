...
void PlanSum::Plan_Processing::Read_Plans (int part)
{
	...
	Dtime time, time2;
	double flow, pce, occ;
	bool flag, drive_flag, transit_flag, veh_type_flag, park_flag, park_demand_flag;
	...
	if (exe->plan_skim_flag) {
		if (!plan_skim_file->Open (part)) {
			MAIN_LOCK
				exe->Error (String ("Opening %s") % plan_skim_file->Filename ());
			END_LOCK
		}
	}
	if (thread_flag) {
	...
	park_demand_flag = exe->System_File_Flag (NEW_PARK_DEMAND);
	...
	while (plan_file->Read_Plan (plan)) {
		...
		if (exe->select_org_zones) {
			...
			if (loc_ptr->Zone () >= 0) {
				...
				if (!exe->org_zone_range.In_Range (zone_ptr->Zone ())) continue;
			}
		}
		if (exe->select_des_zones) {
			...
			if (loc_ptr->Zone () >= 0) {
				zone_ptr = &exe->zone_array [loc_ptr->Zone ()];
				if (!exe->des_zone_range.In_Range (zone_ptr->Zone ())) continue;
			}
		}
		...
		park_flag = (mode == PNR_OUT_MODE || mode == DRIVE_MODE || mode == HOV2_MODE || mode == HOV3_MODE || mode == HOV4_MODE);
		...
		if (exe->plan_stop_flag && transit_flag) {
			int board, alight, route, run, max_rider, max_link, index, stop, capacity;
			double load_fac, factor;
			String field;
			bool type_flag;
			...
			exe->plan_stop_file.Put_Field ("HHOLD", plan.Household ());
			exe->plan_stop_file.Put_Field ("PERSON", plan.Person ());
			exe->plan_stop_file.Put_Field ("TOUR", plan.Tour ());
			exe->plan_stop_file.Put_Field ("TRIP", plan.Trip ());
			exe->plan_stop_file.Put_Field ("START", plan.Depart ());

			exe->plan_stop_file.Put_Field ("STOP1", 0);
			exe->plan_stop_file.Put_Field ("STOP2", 0);
			exe->plan_stop_file.Put_Field ("STOP3", 0);
			exe->plan_stop_file.Put_Field ("STOP4", 0);
			exe->plan_stop_file.Put_Field ("STOP5", 0);

			exe->plan_stop_file.Put_Field ("MAXLINK", 0);
			exe->plan_stop_file.Put_Field ("MAXLOAD", 0);
			exe->plan_stop_file.Put_Field ("LOADFAC", 0);

			board = alight = route = max_link = -1;
			max_rider = index = 0;
			load_fac = 0.0;
			...
			for (leg_itr = plan.begin (); leg_itr != plan.end (); leg_itr++, time += time2) {
				...
				if (leg_itr->Type () == STOP_ID) {
					if (leg_itr->Mode () == TRANSIT_MODE) {
						...
						line_type_ptr = &exe->veh_type_array [line_ptr->Type ()];
						...
						for (stop_itr = board_itr; stop_itr <= alight_itr; stop_itr++) {
							...
							if (exe->Stop_Equiv_Flag ()) {
								...
								for (int i = exe->stop_equiv.First_Group (); i > 0; i = exe->stop_equiv.Next_Group ()) {
									list = exe->stop_equiv.Group_List (i);
							...
							if (stop_itr == board_itr) {
								if (index > 0) {
									...
									if (exe->plan_stop_file.Get_Integer (field) != stop) {
										index++;
										field ("STOP%d") % index;

										exe->plan_stop_file.Put_Field (field, stop);
									}
								} else {
									index = 1;
									exe->plan_stop_file.Put_Field ("STOP1", stop);
								}
							}
							if (stop_itr == alight_itr) {
								field ("STOP%d") % index;
								...
								if (type_flag) {
									run_type_ptr = &exe->veh_type_array [line_ptr->Run_Type (run)];
									capacity = run_type_ptr->Capacity ();
								} else {
									capacity = line_type_ptr->Capacity ();
								}
								if (capacity > 0) {
									factor = (double) run_ptr->Load () / capacity;
								} else {
									factor = run_ptr->Load ();
								}
								if (factor > load_fac) {
									load_fac = factor;
									max_rider = run_ptr->Load ();
									max_link = stop_ptr->Link ();
								}
							}
						}
						board = alight = route = -1;
			...
			if (max_link >= 0) {
				exe->plan_stop_file.Put_Field ("MAXLINK", exe->link_array [max_link].Link ());
				exe->plan_stop_file.Put_Field ("MAXLOAD", max_rider);
				exe->plan_stop_file.Put_Field ("LOADFAC", load_fac);
				exe->plan_stop_file.Write ();
			}
		}

		//---- parking demand ----

		if (park_demand_flag && park_flag) {
			int index;
			bool flag;
			...
			if (index > 0) {
				...
				if (map_itr != exe->parking_map.end ()) {
					duration = 2 * (tod - time) + plan.Activity ();
MAIN_LOCK
					if (exe->park_demand_array.Check_Parking_Type (map_itr->second)) {
						exe->park_demand_array.Parking_Duration (map_itr->second, time, duration);
					}
END_LOCK
				}
			}
		...
		//---- write the plan skim file record ----

		if (exe->plan_skim_flag) {
			plan_skim_file->Put_Data (plan);
