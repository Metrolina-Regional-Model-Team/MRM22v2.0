...
void TransimsNet::Repair_Connections (void)
{
	...
	for (dir_index = 0, dir_itr = dir_array.begin (); dir_itr != dir_array.end (); dir_itr++, dir_index++) {
		...
		for (i=0; i < lanes; i++) {
			if (lane_connect [i] == false) {
				...
				Warning (String ("Node %d Link %d %s Exit Lane %s was Not Repaired") % node_ptr->Node () % link_ptr->Link () % dir % Lane_ID_Code (lane_code));
			}
		}
	}
	...
	for (dir_index = 0, dir_itr = dir_array.begin (); dir_itr != dir_array.end (); dir_itr++, dir_index++) {
		...
		for (i=0; i < lanes; i++) {
			if (lane_connect [i] == false) {
				lane_code = Make_Lane_ID (&(*dir_itr), i);
				Warning (String ("Node %d Link %d %s Entry Lane %s was Not Repaired") % node_ptr->Node () % link_ptr->Link () % dir % Lane_ID_Code (lane_code));
			}
		}
	}
	End_Progress ();
