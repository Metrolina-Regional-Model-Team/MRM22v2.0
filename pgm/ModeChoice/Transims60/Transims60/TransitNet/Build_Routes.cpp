...
void TransitNet::Build_Routes (void)
{
	...
	Dtime offset_time, time_inc, low, high, last_time, min_time;
	...
	bool offset_flag, connect_flag, station_flag;
	Use_Type use;
	String note;
	...
	for (route_itr = route_nodes_array.begin (); route_itr != route_nodes_array.end (); route_itr++) {
		...
		station_flag = (mode != LOCAL_BUS && mode != EXPRESS_BUS);
		...
		for (route_path_itr = route_path_array.begin (); route_path_itr != route_path_array.end (); route_path_itr++) {
			...
			if (first_flag || last_flag || mid_flag) {
				if (link_ptr->Area_Type () == 0) link_ptr->Area_Type (3);
				spacing = Round (min_stop_spacing.Best (link_ptr->Area_Type ()));

				if (!station_flag && spacing > 0 && prev_len > 0 && !end_flag) {
				...
				if (bus_flag && mode != BRT) {
					...
					if (local_access [dir_index] == 0 || short_flag) {
						...
						//}
						Warning (String ("Route %d Stop was Ignored on Link %d %s") % route % link_ptr->Link () % Facility_Code ((Facility_Type) link_ptr->Type ()));
	...
	for (route_itr = route_nodes_array.begin (); route_itr != route_nodes_array.end (); route_itr++) {
		...
		if (line_ptr->size () < 2) {
			Warning (String ("Route %d-%s does not have two or more stops") % route_itr->Route () % route_itr->Name ());
			continue;
		}
		head0 = -1;
		n0 = 0;
		...
		for (p=0, period_itr = route_itr->periods.begin (); period_itr != route_itr->periods.end (); period_itr++, p++) {
			...
			if (period_itr->TTime () > 0 && line_ptr->size () > 0) {
			...
			if (head0 == headway) {
				...
				min_time = ttime = low + offset;
				last_time = last_time - 1.5 * headway;
				if (ttime < last_time) {
					ttime = last_time;
					if ((ttime - min_time) > headway / 2) {
						ttime = min_time + headway / 2;
					}
				}
				n0 = period_itr->Index ();
			}
