...
{
	int node_field, lot_field, capacity_field, hourly_field, daily_field, pnr_access, speed, pnr_index;
	int dir, node, lot, capacity, hourly, daily, lot_offset, min_length, offset, type;
	bool walk_flag, stop_flag, type_flag;
	...
	Parking_Data park_rec, *park_ptr;
	...
	node_field = parkride_file.Optional_Field ("NODE");
	lot_field = parkride_file.Optional_Field ("LOT", "ID", "RECORD");
	...
	if (node_field < 0 && lot_field < 0) {
		Error ("Park and Ride Node or Lot field is Required");
	}
	park_ptr = 0;
	lot_offset = Round (Internal_Units (30, METERS));
	min_length = lot_offset * 3 / 2;
	pnr_access = pnr_index = 0;
	...
	while (parkride_file.Read ()) {
		...
		lot = parkride_file.Get_Integer (lot_field);
		if (node <= 0 && lot <= 0) continue;
		...
		if (lot > 0) {
			map_itr = parking_map.find (lot);
			if (map_itr == parking_map.end ()) {
				Warning (String ("Park&Ride Lot %d was Not Found in the Parking File") % lot);
				parking_warnings++;
				continue;
			}
			pnr_index = map_itr->second;
			park_ptr = &parking_array [map_itr->second];
			park_ptr->Type (PARKRIDE);
			nparkride++;
		} else if (node > 0) {
			...
			offset = 0;
			best_dir = 0;
			walk_flag = stop_flag = type_flag = false;
			...
			for (dir = node_list [map_itr->second]; dir >= 0; dir = dir_list [dir]) {
				...
				if (map_itr != stop_list->end ()) {
					if (best_dir == 0 || !stop_flag || (!type_flag && (map_itr->first < offset ||
						...
						if (stop_type_flag) {
							...
							type_flag = pnr_stop_flag [stop_ptr->Type ()];
						}
					}
			...
			//---- check for an existing lot ----

			if (!park_loc_set.insert (Int2_Key (best_dir->Link_Dir (), offset)).second) {
				Warning (String ("ParkRide Location for Node %d Already Exists") % node);
				parking_warnings++;
				continue;
			}
			nparkride++;
		...
		if (new_access_flag && PNR_distance > 0) {
			link_ptr = &link_array [park_ptr->Link ()];

			Link_Shape (link_ptr, park_ptr->Dir (), points, UnRound (park_ptr->Offset ()), 0.0);
