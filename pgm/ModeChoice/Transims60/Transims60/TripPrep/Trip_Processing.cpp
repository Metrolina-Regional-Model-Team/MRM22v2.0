...
TripPrep::Trip_Processing::Trip_Processing (TripPrep *_exe, int _num)
{
	...
	num = _num;
	num_temp = 0;
	trip_file = script_file = new_trip_file = merge_file = 0;
}
...
void TripPrep::Trip_Processing::operator()()
{
	...
	if (exe->script_flag) {
		script_file = new Trip_File (exe->script_file->File_Access (), exe->script_file->Dbase_Format ());
		script_file->Replicate_Fields (exe->script_file);
		script_file->First_Open (false);
	}
