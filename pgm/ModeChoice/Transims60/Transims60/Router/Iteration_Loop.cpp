...
void Router::Iteration_Loop (void)
{
	...
	Dtime second;
	bool old_flag, duration_flag, last_skip, last_flag, link_last_flag, trip_last_flag, transit_last_flag, park_last_flag;
	...
	for (iteration=1; iteration <= max_iteration; iteration++) {
		...
		link_last_flag = trip_last_flag = transit_last_flag = park_last_flag = true;
		...
		if (Rider_Flag () || (System_File_Flag (RIDERSHIP) && param.cap_penalty_flag)) {
			...
			gap = line_array.Ridership_Gap (param.cap_penalty_flag, initial_factor, !last_flag);
		...
		if (Park_Demand_Flag ()) {
			gap = part_processor.Update_Penalties (!last_flag);

			Write (1, "Parking Capacity Gap  = ") << gap;

			if (parking_gap > 0.0 && gap > parking_gap) park_last_flag = false;
		}
		...
		if (!last_flag && max_iteration > 1 && Report_Flag (ITERATION_PROBLEMS)) {
			Report_Problems (counters.total_records, false);
		}
