...
#endif
{
	...
	max_subarea = num_vehicles = num_on_net = 0;
	num_subareas = num_sims = 1;
	node_offset_factor = 10;
	max_method = 0;
...
void Simulator_Service::Simulator_Service_Keys (int *keys)
{
	Control_Key control_keys [] = { //--- code, key, level, status, type, default, range, help ----
		...
		{ GROUP_PERIOD_METHODS, "GROUP_PERIOD_METHODS", LEVEL1, OPT_KEY, LIST_KEY, "MESOSCOPIC", "NOSIM, MACRO, MESO, MICRO", NO_HELP },
		{ RANDOM_NODE_INCREMENT, "RANDOM_NODE_INCREMENT", LEVEL0, OPT_KEY, TIME_KEY, "1.0 minutes", "0..180.0 minutes", NO_HELP },
		...
		{ MAXIMUM_LANE_USE_WAIT, "MAXIMUM_LANE_USE_WAIT", LEVEL0, OPT_KEY, TIME_KEY, "60 seconds", "0..6500 seconds", NO_HELP },
	...
	int converge_service_keys [] = {
		STORE_PLANS_IN_MEMORY, MAXIMUM_NUMBER_OF_ITERATIONS, LINK_CONVERGENCE_CRITERIA, TRIP_CONVERGENCE_CRITERIA, 
