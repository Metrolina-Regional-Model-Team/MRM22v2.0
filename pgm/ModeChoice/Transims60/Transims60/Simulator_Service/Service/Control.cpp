...
void Simulator_Service::Program_Control (void)
{
	...
	//---- maximum lane use wait ----

	param.max_use_wait = Get_Control_Time (MAXIMUM_LANE_USE_WAIT);
	...
	if (select_problems && param.count_warnings) {
		for (i=1; i < MAX_PROBLEM; i++) {
			if (i != WAIT_PROBLEM && i != USE_PROBLEM && i != ARRIVAL_PROBLEM && i != DEPARTURE_PROBLEM) {
