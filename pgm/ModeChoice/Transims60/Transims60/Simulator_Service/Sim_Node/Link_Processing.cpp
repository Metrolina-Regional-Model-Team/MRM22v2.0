...
int Sim_Node_Process::Link_Processing (int link)
{
	int i, max_exit, exit_count, lane0, lane, cell, traveler, step_size, next_load, vehs;
	double max_flow;
	Dtime time;
	...
	max_exit = exit_count = vehs = 0;

	step.Process_ID (ID ());
	step.Dir_Index (link);
	step.sim_dir_ptr = sim_dir_ptr = &sim->sim_dir_array [link];

//exe->debug = (sim->link_array [sim->dir_array [link].Link ()].Link () == 3870);
if (exe->debug) exe->Write (1, "*****step=") << sim->time_step << " LINK=" << link;
...
if (exe->debug) exe->Write (0, " max_exit=") << max_exit;
...
if (exe->debug) exe->Write (1, " traveler=") << traveler;
			...
			//---- check the use wait constraint ----

			if (sim_travel_ptr->Wait () > sim->param.max_use_wait && sim_travel_ptr->Problem () == USE_PROBLEM) {
