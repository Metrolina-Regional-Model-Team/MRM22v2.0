...
bool Sim_Node_Process::Move_Vehicle (Travel_Step &step)
{
	...
	if (step.sim_veh_ptr == 0) {
		...
		if (sim_veh_ptr != step.sim_veh_ptr) {
			sim->Error ("Simulator_Service::Move_Vehicle: Vehicle Pointer");
		}
#endif
	}
...
#ifdef DUMP_DETAILS
	if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Move dir=" << dir_index << " speed=" << speed << " max_speed=" << max_speed << " leg_type=" << leg_ptr->Type ();
#endif
#ifdef CHECK
...
if (exe->debug) exe->Write (0, " cell=") << lane << "-" << cell << " spd=" << speed;

	step.push_back (sim_veh);

#ifdef DUMP_DETAILS
	if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Remove dir=" << dir_index << " lane=" << lane << " cell=" << cell << " offset=" << offset << " = " << sim_dir_ptr->Get (lane, cell);
#endif
	sim_dir_ptr->Remove (lane, cell);
#ifdef DUMP_DETAILS
	if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Cell dir=" << dir_index << " lane=" << lane << " cell=" << cell << " = " << sim_dir_ptr->Get (lane, cell);
#endif
	connect_ptr = 0;
	...
	if (sim_dir_ptr->Method () == MACROSCOPIC) {
		max_move = Gap_Check (sim_dir_ptr, max_move, lane, cell);
	}

#ifdef DUMP_DETAILS
	if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Move dir=" << dir_index << " max_move=" << max_move;
#endif
	//---- process each movement increment ----

	for (move = 0; move <= max_move; move++) {
		new_link = next_leg_flag = slow_down = false;
		step.New_Link (new_link);
...
if (exe->debug) exe->Write (0, " FLAG");
	...
	if (exe->debug) exe->Write (0, " enter=") << low << "-" << high;
			}
		...
		if (!move_flag) {
			if (!new_link) {
				...
				if (move_flag) {
	if (exe->debug) exe->Write (0, " change lane=") << lane;
					keep_flag = true;
				}
			} else {
				if (exe->debug) {
					...
					for (l = low; l <= high; l++) {
						exe->Write (0, " l=") << l << "=" << new_dir_ptr->Get (l, cell);
					}
				}
				break;
			}
...
#ifdef DUMP_DETAILS
		if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Move dir=" << dir_index << " lane=" << lane << " offset=" << offset;
#endif
		sim_veh.Location (dir_index, lane, offset);
	...
	*sim_veh_ptr = sim_veh;
...
#ifdef DUMP_DETAILS
		if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Veh dir=" << sim_veh.link << " lane=" << sim_veh.lane << " offset=" << sim_veh.offset;
		if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Locate dir=" << dir_index << " lane=" << lane << " cell=" << cell << " offset=" << sim_veh.offset;
		if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Cells=" << veh_type_ptr->Cells () << " size=" << step.size () << " offset_cell=" << sim->Offset_Cell (step [0].offset);
#endif
			...
			for (++index, sim_veh_itr = to_step.begin (); sim_veh_itr != to_step.end (); sim_veh_itr++, index++) {
...
#ifdef DUMP_DETAILS
					if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Remove dir=" << dir_index << " lane=" << lane << " cell=" << cell << " offset=" << sim_veh_ptr->offset << " traveler=" << sim_dir_ptr->Get (lane, cell);
#endif
				...
				*sim_veh_ptr = *sim_veh_itr;
				...
				if (!sim_veh_ptr->Parked ()) {
					...
					cell = sim->Offset_Cell (sim_veh_ptr->offset);
#ifdef DUMP_DETAILS
					if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Add dir=" << dir_index << " lane=" << lane << " cell=" << cell << " offset=" << sim_veh_ptr->offset << " traveler=" << sim_dir_ptr->Get (lane, cell) << " new_index=" << new_index;
#endif
					if (sim_dir_ptr->Get (lane, cell) != step.Traveler ()) {
	...
	if (step.Speed () > 0) {
		sim_travel_ptr->Speed (step.Speed ());
	} else {
...
#ifdef DUMP_DETAILS
	if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Call Output_Step=" << step.size () << " dir_index=" << step.Dir_Index () << " traveler=" << step.Traveler () << " veh=" << step.sim_travel_ptr->Vehicle ();
#endif
