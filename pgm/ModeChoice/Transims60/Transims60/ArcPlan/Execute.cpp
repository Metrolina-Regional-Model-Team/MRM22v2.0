...
void ArcPlan::Execute (void)
{
	...
	if (plan_skim_flag && drive_access_flag) {
		arcview_drive_access.Close ();
	}
	...
	if (location_flag) {
		productions.assign (location_array.size (), 0);
		attractions.assign (location_array.size (), 0);
	}
	...
	if (plan_flag && drive_access_flag) {
		arcview_drive_access.Close ();
	}
	...
	if (location_flag) Write (1, "Number of Arc Location Demand Shape Records = ") << num_location;
	if (drive_access_flag) Write (1, "Number of Arc Drive Access Shape Records = ") << num_drive_access;
