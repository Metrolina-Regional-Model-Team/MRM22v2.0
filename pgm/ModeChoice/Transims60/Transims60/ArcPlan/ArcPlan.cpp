...
ArcPlan::ArcPlan (void) : Data_Service (), Select_Service (), Draw_Service ()
{
	...
	System_File_Type optional_files [] = {
		SHAPE, ACCESS_LINK, ZONE, 
		TRANSIT_STOP, TRANSIT_ROUTE, TRANSIT_DRIVER, VEHICLE_TYPE, 
		PLAN, PROBLEM, SELECTION, PLAN_SKIM, END_FILE
	};
	...
	int select_service_keys [] = {
		...
		SELECT_TRAVELER_TYPES, SELECT_FACILITY_TYPES, SELECT_PARKING_LOTS, SELECT_TRANSIT_MODES, SELECT_ROUTES, SELECT_STOPS, 
	...
	Control_Key arcplan_keys [] = {  //--- code, key, level, status, type, default, range, help ----
		...
		{ NEW_ARC_LOCATION_DEMAND_FILE, "NEW_ARC_LOCATION_DEMAND_FILE", LEVEL0, OPT_KEY, OUT_KEY, "", FILE_RANGE, NO_HELP },
		{ NEW_ARC_DRIVE_ACCESS_FILE, "NEW_ARC_DRIVE_ACCESS_FILE", LEVEL0, OPT_KEY, OUT_KEY, "", FILE_RANGE, NO_HELP },
		...
		{ DRIVE_ACCESS_DISPLAY_METHOD, "DRIVE_ACCESS_DISPLAY_METHOD", LEVEL0, OPT_KEY, TEXT_KEY, "VECTOR", "VECTOR, POINTS, ACCESS", NO_HELP },
		END_CONTROL
	};
	const char *reports [] = {
		"STOP_EQUIVALENCE",
	...
	num_out = num_problem = max_problem = num_width = num_time = num_distance = num_access = num_drive_access = 0;
	num_rider = num_stop = num_group = num_parking = num_location = problem_method = 0;
	drive_parking = -1;
	...
	rider_flag = load_flag = on_off_flag = demand_flag = group_flag = parking_flag = location_flag = false;
	mode_flag = random_flag = width_flow_flag = drive_access_flag = plan_skim_flag = false;
