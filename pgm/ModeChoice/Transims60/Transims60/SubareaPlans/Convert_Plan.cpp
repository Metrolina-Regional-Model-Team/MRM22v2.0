	...
	if (drive_flag) {
		...
		if (new_flag && sub_flag) {
			...
			for (leg_itr = plan.begin (); leg_itr != plan.end (); leg_itr++, tod += time) {
					...
					if (map_itr == link_map.end ()) {
						...
						if (sub_flag) {
							...
							if (leg_ptr->Link_Type ()) {
								...
								if (map_itr == link_map.end ()) {
									Warning ("Plan ") << plan.Household () << " Link=" << leg_ptr->Link_ID () << " is Not in the Network";
									return;
								}
								...
								if (dir < 0) {
									Warning ("Plan ") << plan.Household () << " Link=" << leg_ptr->Link_ID () << " Direction=" << leg_ptr->Link_Dir () << " Not in the Network";
									return;
								}
								data_ptr = &sublink_array [dir];

								if (data_ptr->parking == 0 || data_ptr->location == 0) {
									Warning ("Plan ") << plan.Household () << " Link=" << link_ptr->Link () << " does not have Parking or Locations";
								}
				...
				} else if (!sub_flag && leg_itr->Type () == LOCATION_ID) {
					if (location_map.find (leg_itr->ID ()) != location_map.end ()) {
						sub_flag = true;
					}
				}
				if (sub_flag) {
