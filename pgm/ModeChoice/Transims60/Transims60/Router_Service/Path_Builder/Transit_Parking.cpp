...
{
	int n, index, lot, org_x, org_y, des_x, des_y, tod, cost, imped, ttime, org_loc, des_loc, max_dist_imp, penalty;
	...
	if (path_param.value_time > 0) {
		max_dist_imp = DTOI (Dtime (60, MINUTES) * path_param.value_time);
	} else {
		max_dist_imp = 1000000;
	}
	...
	for (; park_type <= num_types; park_type++) {
		...
		for (lot_itr = lot_map->begin (); lot_itr != lot_map->end (); lot_itr++) {
			...
			if (park_flag) {
				...
				if (park_ptr->Type () != park_type && !(park_ptr->Type () == MAJOR_PNR && park_type == RAIL_PNR)) continue;
				if (path_param.park_pen_flag) {
					if (exe->park_demand_array.Penalty (parking, tod) >= MAX_PENALTY) continue;
				}
			}
		...
		for (n = 0, lot_dist_itr = lot_dist_map.begin (); lot_dist_itr != lot_dist_map.end () && n < num_lots; lot_dist_itr++, n++) {
			...
			if (n == 0) {
				...
				if (imped > max_dist_imp) imped = max_dist_imp;
			}

			park_ptr = &exe->parking_array [parking];

			if (park_type == RAIL_PNR && park_ptr->Type () == MAJOR_PNR) continue;
			...
			if (park_flag) {
#ifdef DUMP_DETAILS
				if (exe->dump_pnr_flag) {
					if (drive_flag) {
						exe->dump_pnr.File () << org_loc << "\t" << des_loc;
					} else {
						exe->dump_pnr.File () << des_loc << "\t" << org_loc;
					}
					exe->dump_pnr.File () << "\t" << park_ptr->Parking () << "\t" << park_ptr->Type () << "\t" << imped << endl;
				}
#endif
				tod = des_itr->Time ();
				if (out_flag) {
				...
				if (out_flag && path_param.park_pen_flag) {
					penalty = exe->park_demand_array.Penalty (parking, tod);
