...
{
	int i, mode, index, dir_index, nlegs, len, cost, rate, imp, use_index, group, penalty, stop;
	Dtime time, ttime, delay, tod;
	bool first_park, ab_flag, flag;
	double factor;

	Plan_Leg *leg_ptr, *leg2_ptr;
	...
	if (mode == WAIT_MODE || mode == WALK_MODE || mode == BIKE_MODE || mode == RIDE_MODE || mode == OTHER_MODE) return (true);
	if (mode == TRANSIT_MODE && exe->Rider_Flag ()) return (true);

	//---- set the traveler parameters ----

	exe->Set_Parameters (path_param, *plan_ptr);
	...
	for (i=0; i < nlegs; i++) {
		...
		if (mode == DRIVE_MODE) {
			...
			if (leg_ptr->Type () == ROUTE_ID && exe->Rider_Flag () && path_param.transit_penalty) {
				index = leg_ptr->ID ();
				leg2_ptr = &plan_ptr->at (i-1);
				stop = leg2_ptr->ID ();
				...
				for (line_stop_itr = line_ptr->begin (); line_stop_itr != line_ptr->end (); line_stop_itr++) {
					if (line_stop_itr->Stop () == stop) {
						for (run_itr = line_stop_itr->begin (); run_itr != line_stop_itr->end (); run_itr++) {
							if (run_itr->Schedule () > tod) {
								if (run_itr->Penalty () > 0) {
									ttime = run_itr->Schedule () - tod + run_itr->Penalty ();

									if (ttime > path_param.max_wait) {
										plan_ptr->Priority (CRITICAL);
									}
									imp = Resolve (ttime * path_param.value_wait);
									break;
								}
							}
						}
			...
			if (leg_ptr->Type () == PARKING_ID) {
				...
				flag = ((forward_flag && !first_park) || (!forward_flag && first_park));

				if (!flag) {
					cost = 0;
				} else {
					cost = parking_ptr->Cost ();
				}
					...
					for (park_itr = parking_ptr->begin (); park_itr != parking_ptr->end (); park_itr++) {
						if (exe->Use_Permission (park_itr->Use (), path_param.use) &&
							...
							if (!flag) {
								...
								rate = DTOI (park_itr->Hourly () * parking_duration.Hours ());
								if (rate > park_itr->Daily ()) rate = park_itr->Daily ();
								if (rate > 0) cost = rate;
							}
							break;
						}
				...
				if (mode == KNR_IN_MODE || mode == KNR_OUT_MODE) {
					cost = 0;
				} else {
					cost = Round (cost);
				}
				imp = DTOI (ttime * path_param.value_park + cost * path_param.value_cost);

				//---- add the parking penalty ----

				if (flag && path_param.park_pen_flag) {
					penalty = exe->park_demand_array.Penalty (index, time);
