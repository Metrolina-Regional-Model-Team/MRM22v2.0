...
{
	int route, stop, s, nstop, index, path, wait, run, imped, xfer, to, to_stop, from_offset, mode, on_zone, off_zone, fare, min_fare;	
	int from_cost, from_len, from_walk, to_cost, to_len, to_walk, wait_cost, wait_len, wait_walk, len, imp;
	...
	Stop_Data *stop_ptr;
	Fare_Index fare_index, wait_index, xfer_index;
	Fare_Map_Itr fare_itr, xfer_itr;
	...
	mode = on_zone = off_zone = fare = 0;

	if (xfer >= path_param.max_xfers) {
		transfer_flag = true;
		return (exit_flag);
	} else if (exe->Fare_Flag ()) {
		if (xfer > 0) {
			if (!Transfer_Fare (path_ptr, mode, on_zone, off_zone, fare)) {
				mode = on_zone = off_zone = fare = 0;
			}
		}
	}
	...
	for (index = exe->stop_list [stop]; index >= 0; index = route_stop_ptr->Next ()) {
		...
		if (forward_flag) {
			...
			if (exe->Fare_Flag ()) {
				wait_index.From_Mode (mode);
				wait_index.From_Zone (off_zone);
				wait_index.To_Mode (line_ptr->Mode ());
				wait_index.To_Zone (line_stop_ptr->Zone ());
				wait_index.Class (path_param.fare_class);

				fare_index.From_Mode (line_ptr->Mode ());
				fare_index.From_Zone (line_stop_ptr->Zone ());
				fare_index.To_Mode (line_ptr->Mode ());
				fare_index.Class (path_param.fare_class);

				xfer_index.From_Mode (mode);
				xfer_index.From_Zone (on_zone);
				xfer_index.To_Mode (line_ptr->Mode ());
				xfer_index.Class (path_param.fare_class);
			}
			...
			for (run=0, run_itr = line_stop_ptr->begin (); run_itr != line_stop_ptr->end (); run_itr++, run++) {
				...
				if (exe->Fare_Flag ()) {
					fare_index.Period (exe->fare_map.Period (run_itr->Schedule ()));
					wait_index.Period (fare_index.Period ());
					xfer_index.Period (fare_index.Period ());

					fare_itr = exe->fare_map.find (wait_index);

					if (fare_itr != exe->fare_map.end ()) {
						wait_cost += fare_itr->second;
						imp = DTOI (fare_itr->second * path_param.value_cost) + wait_imp;
						if (imp < 0) imp = 0;
						wait_imp = imp;
						if (wait_imp >= max_imp) continue;
					}
				}
				to_walk = from_walk;
				...
				for (++s; s < nstop; s++) {
					...
					if (exe->Fare_Flag ()) {
						fare_index.To_Zone (to_stop_ptr->Zone ());
						xfer_index.To_Zone (to_stop_ptr->Zone ());

						fare_itr = exe->fare_map.find (fare_index);

						if (fare_itr != exe->fare_map.end ()) {
							min_fare = fare + fare_itr->second;

							if (xfer_index.From_Mode () == xfer_index.To_Mode ()) {
								xfer_itr = exe->fare_map.find (xfer_index);

								if (xfer_itr != exe->fare_map.end ()) {
									if (xfer_itr->second < min_fare) {
										min_fare = xfer_itr->second;
									}
								}
							}
							min_fare -= fare;
							to_cost += min_fare;
							imp = DTOI (min_fare * path_param.value_cost) + to_imp;
							if (imp < 0) imp = 0;
							to_imp = imp;
							if (to_imp >= max_imp) continue;
						}
					...
					if (path_param.sort_method) {
						...
						if (path_ptr->Status () == 0) {
							next_index.Put_Back (path_index);
			...
			if (exe->Fare_Flag ()) {
				wait_index.To_Mode (mode);
				wait_index.To_Zone (off_zone);
				wait_index.From_Mode (line_ptr->Mode ());
				wait_index.From_Zone (line_stop_ptr->Zone ());
				wait_index.Class (path_param.fare_class);

				fare_index.To_Mode (line_ptr->Mode ());
				fare_index.To_Zone (line_stop_ptr->Zone ());
				fare_index.From_Mode (line_ptr->Mode ());
				fare_index.Class (path_param.fare_class);

				xfer_index.To_Mode (mode);
				xfer_index.To_Zone (on_zone);
				xfer_index.From_Mode (line_ptr->Mode ());
				xfer_index.Class (path_param.fare_class);
			}
			...
			for (run_ritr = line_stop_ptr->rbegin (); run_ritr != line_stop_ptr->rend (); run_ritr++, run--) {
				...
				if (exe->Fare_Flag ()) {
					fare_index.Period (exe->fare_map.Period (run_ritr->Schedule ()));
					wait_index.Period (fare_index.Period ());
					xfer_index.Period (fare_index.Period ());
				...
				for (--s; s >= 0; s--) {
					...
					if (exe->Fare_Flag ()) {
						fare_index.From_Zone (to_stop_ptr->Zone ());
						xfer_index.From_Zone (to_stop_ptr->Zone ());

						fare_itr = exe->fare_map.find (fare_index);

						if (fare_itr != exe->fare_map.end ()) {
							min_fare = fare_itr->second + fare;
					...
					if (path_param.sort_method) {
						...
						if (path_ptr->Status () == 0) {
							...
							next_index.Put_First (path_index);
						}
					}
					path_ptr->Imped (to_imp);
					path_ptr->Time (to_time);
