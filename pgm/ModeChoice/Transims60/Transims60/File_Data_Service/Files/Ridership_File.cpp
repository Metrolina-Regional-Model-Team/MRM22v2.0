...
void Ridership_File::Setup (void)
{
	...
	mode = route = run = stop = schedule = time = penalty = board = alight = load = factor = -1;
}
...
bool Ridership_File::Create_Fields (void) 
{
	...
	Add_Field ("PENALTY", DB_TIME, TIME_FIELD_SIZE, SECONDS);
...
bool Ridership_File::Set_Field_Numbers (void)
{
	...
	penalty = Optional_Field ("PENALTY", "WAIT");
