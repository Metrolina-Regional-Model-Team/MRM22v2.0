...
void Parking_File::Setup (void)
{
	...
	parking = link = dir = offset = type = capacity = cost = -1;
...
bool Parking_File::Create_Fields (void) 
{
	...
	Add_Field ("CAPACITY", DB_INTEGER, 6, VEHICLES);
	Add_Field ("COST", DB_INTEGER, 6, CENTS);
	...
	Add_Field ("SPACE", DB_UNSIGNED, 2, VEHICLES, true, NESTED);
...
bool Parking_File::Set_Field_Numbers (void)
{
	...
	capacity = Optional_Field ("CAPACITY", "VEHICLES", "MAX_VEH");
	cost = Optional_Field ("COST", "CHARGE");

	use = Optional_Field ("USE", "VEHICLE");
	start = Optional_Field (START_FIELD_NAMES);
	end = Optional_Field (END_FIELD_NAMES);
	space = Optional_Field ("SPACE", "SPACES", "MAX_SPACE");
