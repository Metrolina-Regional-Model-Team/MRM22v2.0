...
File_Service::File_Key  File_Service::file_keys [] = {
	...
	{ PARK_DEMAND, "PARKING_DEMAND_FILE", "PARKING_DEMAND_FORMAT" },
	{ PLAN_SKIM, "PLAN_SKIM_FILE", "PLAN_SKIM_FORMAT" },
	...
	{ NEW_PARK_DEMAND, "NEW_PARKING_DEMAND_FILE", "NEW_PARKING_DEMAND_FORMAT" },
	{ NEW_PLAN_SKIM, "NEW_PLAN_SKIM_FILE", "NEW_PLAN_SKIM_FORMAT" },
};
...
File_Service::File_Service (void) : Execution_Service ()
{
	...
	lane_use_flows = trip_sort_flag = time_sort_flag = false;
...
void File_Service::Program_Control (void)
{
	...
	if (Control_Key_Status (TRIP_SORT_TYPE) || Control_Key_Status (PLAN_SORT_TYPE)) {
		if (!Check_Control_Key (TRIP_SORT_TYPE) && Check_Control_Key (PLAN_SORT_TYPE)) {
			Trip_Sort (Trip_Sort_Code (Get_Control_String (PLAN_SORT_TYPE)));
		} else {
			Trip_Sort (Trip_Sort_Code (Get_Control_String (TRIP_SORT_TYPE)));
		}
	...
	for (i=0; i < END_FILE; i++) {
		...
		switch (i) {
			...
			case NEW_TRIP:
				...
				if (i == NEW_TRIP && Control_Key_Status (TRIP_SORT_TYPE) && Check_Control_Key (TRIP_SORT_TYPE)) {
					Get_Control_Text (TRIP_SORT_TYPE);
					Trip_File *fh = (Trip_File *) file->file;
					fh->Sort_Type (Trip_Sort ());
					fh->Update_Def_Header ();
				}
				break;
			case PROBLEM:
			case NEW_PROBLEM:
			...
			case NEW_PLAN:
				...
				if (i == NEW_PLAN && Control_Key_Status (PLAN_SORT_TYPE) && Check_Control_Key (PLAN_SORT_TYPE)) {
					Get_Control_Text (PLAN_SORT_TYPE);
					Plan_File *fh = (Plan_File *) file->file;
					fh->Sort_Type (Trip_Sort ());
					fh->Update_Def_Header ();
				}
				break;
			...
			case NEW_TRAVELER:
				...
				break;
			case PARK_DEMAND:
			case NEW_PARK_DEMAND:
				file->file = new Park_Demand_File (key, file->access, format);
				break;
			case PLAN_SKIM:
			case NEW_PLAN_SKIM:
				file->file = new Plan_Skim_File (key, file->access, format);
				if (i == NEW_PLAN_SKIM && Control_Key_Status (PLAN_SORT_TYPE) && Check_Control_Key (PLAN_SORT_TYPE)) {
					Get_Control_Text (PLAN_SORT_TYPE);
					Plan_Skim_File *fh = (Plan_Skim_File *) file->file;
					fh->Sort_Type (Trip_Sort ());
					fh->Update_Def_Header ();
				}
				break;
			default:
				Error (String ("System File Type %d was Not Initialized") % i);
				break;
		}
	}
