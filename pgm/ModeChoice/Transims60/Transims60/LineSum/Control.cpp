		...
		for (i=1; i <= num_files; i++) {
			...
			if (!key.empty ()) {
				...
				if (on_off_report_data.arcview_flag) {
					...
					on_off_report_data.arc_file->Set_Projection (projection);
		...
		for (i=1; i <= num_files; i++) {
			...
			if (!key.empty ()) {
				...
				if (access_report_data.arcview_flag) {
					...
					access_report_data.arc_file->Set_Projection (projection);;
	...
	if (num_files > 0) {
		...
		for (i=1; i <= num_files; i++) {
			...
			if (link_rider_data.arcview_flag) {
				...
				link_rider_data.arc_file->Set_Projection (projection);
		...
		if (arcview_flag) {
			...
			if (!key.empty ()) {
				...
				link_shape.Set_Projection (projection);
