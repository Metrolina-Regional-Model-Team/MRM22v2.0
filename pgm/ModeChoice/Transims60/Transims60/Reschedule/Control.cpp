...
void Reschedule::Program_Control (void)
{
	...
	if (flag) {
		...
		//---- set the minimum mode percents ----

		if (Check_Control_Key (MINIMUM_MODE_PERCENTS)) {
			Get_Control_List_Groups (MINIMUM_MODE_PERCENTS, mode_min);
			min_flag = true;
		}

		//---- set the maximum mode percents ----

		if (Check_Control_Key (MAXIMUM_MODE_PERCENTS)) {
			Get_Control_List_Groups (MAXIMUM_MODE_PERCENTS, mode_max);
			max_flag = true;
		}

		if (min_flag && max_flag) {
			for (i = 0; i < ANY_TRANSIT; i++) {
				if (mode_min.Best (i) > mode_max.Best (i)) {
					Warning (String ("Illogical Min/Max Percents for Mode=%d (%.1lf > %.1lf)") % i %  mode_min.Best (i) % mode_max.Best (i));
				}
			}
		}
	}
	...
	if (mode_factor.size () > 0) {
		...
		for (i = 1; i < ANY_TRANSIT; i++) {
			...
			if (min_flag) {
				Print (0, String ("  Minimum=%.1lf%%") % mode_min.Best (i) % FINISH);
			}
			if (max_flag) {
				Print (0, String ("  Maximum=%.1lf%%") % mode_max.Best (i) % FINISH);
			}
		}
	}
