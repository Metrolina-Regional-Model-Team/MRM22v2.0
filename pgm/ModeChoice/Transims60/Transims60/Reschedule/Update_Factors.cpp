...
void Reschedule::Update_Factors (void)
{
	...
	double len_fac, factor, min_percent, max_percent, percent;
	Dtime time, min_time, sum, tod, start, old_tod, new_tod, old_time;
	bool first;
				...
				for (; stop_itr != line_itr->end (); stop_itr++) {
					...
					if (!first) {
						...
						if ((min_flag || max_flag) && old_time > 0) {
							percent = 100.0 * (sum - old_time) / old_time;
							if (percent < min_percent) {
								percent = min_percent;
							} else if (percent > max_percent) {
								percent = max_percent;
							}
							sum = old_time + (int) (old_time * percent / 100.0);
						}
