...
{
	int i, num_targets, num_seg, seg, s1, s2, mode, nest, model;
	...
	calib_model_flag = (target_file.Field_Number (model_names [first_model]) >= 0);

	if (mode_field < 0 && target_field < 0 && segment_field < 0) {
		calib_model_flag = false;
	...
	} else if (mode_field < 0 || (target_field < 0 && !calib_model_flag)) {
		goto field_error;
	}
	...
	while (target_file.Read ()) {
		...
		if (target_field >= 0) {
			target = target_file.Get_Integer (target_field) * scale_fac;
		}
		if (calib_model_flag) {
			target = 0;

			for (model = first_model; model < num_models; model++) {
				target += sum = target_file.Get_Integer (model_names [model]) * scale_fac;

				calib_target [seg] [mode] [model] = sum;
			}
		}
		calib_target [seg] [mode] [num_models] = target;
		num_targets++;
		...
		for (i = num_nests - 1; i >= 0; i--) {
			...
			for (model = first_model; model <= num_models; model++) {
				...
				for (mode_itr = nest_ptr->begin (); mode_itr != nest_ptr->end (); mode_itr++) {
					sum += calib_target [seg] [*mode_itr] [model];
				}
				if (nest < 0) {
					calib_target [seg] [num_modes] [model] += sum;
				} else {
					calib_target [seg] [nest] [model] = sum;
				}
			}
		}
	}
	...
	if (new_calib_flag) {
		...
		for (i=first_model; i < num_models; i++) {
			calib_file.Add_Field (model_names [i], DB_DOUBLE, 12.6);
		}
...
void ModeChoice::Target_Header (void)
{
	...
	if (calib_model_flag) {
		for (i=first_model; i < num_models; i++) {
			Print (0, String ("%8.8s ") % model_names [i]);
		}
	}
