...
{
	int *start, *end, *cap_start, *cap_end, stop, index, load, runs, run, capacity, cap, period, count, length;
	...
	for (map_itr = line_map.begin (); map_itr != line_map.end (); map_itr++) {
		...
		if (count > 0) {
			Warning (String ("Route %d %s has %d links (%.1lf miles) beyond the last stop") % line_ptr->Route () % line_ptr->Name () % count % External_Units (UnRound (length), MILES));
		}
	}
	End_Progress ();
}
