...
void ArcRider::Program_Control (void)
{
	...
	if (!key.empty ()) {
		...
		arcview_line.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_sum.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_rider.Add_Field ("PENALTY", DB_TIME, 10, SECONDS);
	...
	if (!key.empty ()) {
		...
		arcview_cap.Set_Projection (projection);
