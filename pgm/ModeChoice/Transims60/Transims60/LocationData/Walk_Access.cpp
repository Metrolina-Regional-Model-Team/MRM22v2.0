...
{
	int distance, runs, stop, loc, x, y;
	double dx, dy, dist;
	Dtime time;
	...
	//---- search for locations near stations ----
	...
	for (stop = 0, int_itr = stop_runs.begin (); int_itr != stop_runs.end (); int_itr++, stop++) {
		...
		stop_ptr = &stop_array [stop];
		if (stop_ptr->Type () != STATION) continue;

		xy_map_itr = stop_xy.find (stop);
		if (xy_map_itr == stop_xy.end ()) continue;

		x = xy_map_itr->second.x;
		y = xy_map_itr->second.y;
		...
		for (loc = 0, loc_itr = location_array.begin (); loc_itr != location_array.end (); loc_itr++, loc++) {
			...
			distance = walk_distance - Resolve (dist);
