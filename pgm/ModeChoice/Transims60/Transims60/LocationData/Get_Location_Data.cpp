...
bool LocationData::Get_Location_Data (Location_File &file, Location_Data &data)
{
	if (Data_Service::Get_Location_Data (file, data)) {
		...
		if (boundary_flag && (zone < 1 || !range_flag || zone_range.In_Range (zone))) {
			double x, y;
			...
			if ((boundary_itr != boundary_map.end () && !In_Polygon (boundary_itr->second, x, y)) ||
				...
				if (zone <= 0) {
					...
					if (System_Data_Flag (ZONE)) {
						int z;
						double dx, dy, diff, best_diff;
						Zone_Itr zone_itr;

						best_diff = 0;

						for (z = 0, zone_itr = zone_array.begin (); zone_itr != zone_array.end (); zone_itr++, z++) {
							if (range_flag && zone_range.In_Range (zone_itr->Zone ())) continue;

							dx = UnRound (zone_itr->X ()) - x;
							dy = UnRound (zone_itr->Y ()) - y;
