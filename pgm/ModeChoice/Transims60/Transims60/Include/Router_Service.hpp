...
public:
	...
	void Rider_Flag (bool flag)                   { rider_flag = flag; }
	bool Rider_Flag (void)                        { return (rider_flag); }

	void Park_Demand_Flag (bool flag)             { park_demand_flag = flag; }
	bool Park_Demand_Flag (void)                  { return (park_demand_flag); }

	double Skim_Gap (void);

	void Set_Parameters (Path_Parameters &param, Trip_Data &trip);
	void Set_Parameters (Path_Parameters &param, int traveler_type = 0, int veh_type = -1, int mode = 0, unsigned traveler = 0);
...
protected:
	enum Router_Service_Keys { 
		...
		MAX_NUMBER_OF_TRANSFERS, CASH_FARE_PERCENTS, CARD_FARE_PERCENTS, SPECIAL_FARE_PERCENTS, 
		NUM_PARK_RIDE_OPTIONS, MAX_PARK_RIDE_PERCENTS, BUS_PARK_RIDE_OPTIONS, BUS_PARK_RIDE_PERCENTS, 
		RAIL_PARK_RIDE_OPTIONS, RAIL_PARK_RIDE_PERCENTS, MAJOR_PARK_RIDE_PERCENTS, MAJOR_PARK_RIDE_OPTIONS, 
		DRIVE_DISTANCE_PENALTIES, MAX_KISS_RIDE_PERCENTS, NUM_KISS_RIDE_OPTIONS, KISS_RIDE_TIME_FACTORS, 
		KISS_RIDE_STOP_TYPES, MAX_KISS_RIDE_DROPOFF_WALK, TRANSIT_PENALTY_FILE, DEFAULT_PARKING_DURATION, 
...
private:
	...
	Integer_List max_parkride, num_parkride, max_bus_pnr, num_bus_pnr, max_rail_pnr, num_rail_pnr, max_major_pnr, num_major_pnr;
	Integer_List max_kissride, num_kissride, max_walk, walk_pen, max_bike, bike_pen;
	Integer_List max_xfers, rail_const, brt_const, bus_const, cash_percents, card_percents, special_percents;
	Dtime_List max_wait, wait_pen, min_wait;
	Double_List freeway_fac, express_fac, kissride_fac, drive_penalty, rail_bias, brt_bias, bus_bias, walk_fac, bike_fac, wait_fac;

	Db_Header stop_pen_file;

	//---- routing specific network data ----

	bool walk_path_flag, bike_path_flag, drive_path_flag, access_link_flag, rider_flag, park_demand_flag;
	bool transit_path_flag, parkride_path_flag, kissride_path_flag, skim_check_flag, fare_class_flag, fare_list_flag;
	double fare_class [SPECIAL + 1];
