...
public:
	...
	void Clear (void)
	{
		length = 0; speed = setback = in_offset = max_cell = out_cell = load_cell = count = subarea = 0; 
		dir = lanes = method = turn = type = status = 0; first_load = last_load = control = -1; thru.clear ();
		Clear_Use ();
	}
