...
public:
	...
	{
		if (file.Write (&first_free, sizeof (first_free))) {
			if (file.Write (&free_count, sizeof (free_count))) {
				return (Vector <Type>::Pack (file));
			}
		}
		return (false);
	}
	...
	{
		if (file.Read (&first_free, sizeof (first_free))) {
			if (file.Read (&free_count, sizeof (free_count))) {
				return (Vector <Type>::UnPack (file));
			}
		}
		return (false);
	}
