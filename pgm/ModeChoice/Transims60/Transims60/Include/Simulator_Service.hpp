...
protected:
	enum Simulator_Service_Keys { 
		...
		PRIORITY_WAITING_TIME, MAXIMUM_WAITING_TIME, MAXIMUM_LANE_USE_WAIT, MAX_DEPARTURE_TIME_VARIANCE, 
	...
	void Set_Vehicle_Locations (void);
	...
	int  Num_On_Net (void)                 { return (num_on_net); }
	void Num_On_Net (int value)            { num_on_net = value; }
	void Add_On_Net (int value = 1)        { num_on_net += value; }
...
private:
	int num_subareas, max_subarea, num_sims, num_vehicles, num_on_net, transit_id;
	int num_travelers, average_legs, step_code, node_offset_factor;
	double avg_cell_per_veh;
	bool active;

	Integers node_link, link_list, gap_speed;
	Integers node_status, node_cells, node_vehs;
	Ints_Array node_nodes;
