...
public:
	...
	int      Load_Func (void)          { return (load_func); }
	...
	void     Load_Func (int value)     { load_func = value; }
	...
	void     Clear (void)
	{
		type = length = max_speed = max_accel = max_decel = capacity = max_load = load_func = occupancy = loading = unloading = method = min_dwell = max_dwell = op_cost = 0;
