...
void Converge_Service::Write_Plan_Files (void)
{
		...
		if (time_order_flag) {
			...
			for (index=0, plan_itr = plan_array.begin (); plan_itr != plan_array.end (); plan_itr++, index++) {
				...
				if (plan_itr->Problem () > 0) {
					...
					if (problem_flag) {
						Plan_Data plan = *(plan_itr);
						if (plan.External_IDs ()) {
							if (problem_set_flag) {
								i = plan.Partition ();
								Write_Problem (problem_set [i], &plan);
							} else {
								Write_Problem (problem_file, &plan);
							}
						}
					}
				}
	...
	if (new_plan_flag) {
		...
		Show_Message (String ("Writing %s -- Record") % new_plan_skim_file->File_Type ());
	}
	Set_Progress ();
	...
	for (first = true; ; first = false) {
		...
		if (!plan_ptr->Path_Problem () && plan_ptr->size () > 0) {
			...
			if (new_plan_skim_flag) {
				new_plan_skim_file->Put_Data (*plan_ptr);
