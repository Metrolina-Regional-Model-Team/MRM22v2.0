...
Converge_Service::Converge_Service (void) : Router_Service (), Select_Service ()
{
	...
	;
	initial_factor = factor_increment = maximum_factor = link_gap = trip_gap = transit_gap = parking_gap = 0.0;
	reroute_time = 0;
	...
	num_time_updates = num_trip_parts = 0;
	percent_selected = 0.0;
	minimum_fuel = seek_fuel = set_mode = set_veh_type = 0;
	...
	new_set_flag = problem_set_flag = trip_memory_flag = plan_memory_flag = preload_flag = info_flag = set_mode_flag = false;
	plan_set_flag = update_flag = select_flag = flow_flag = turn_flag = time_order_flag = time_output_flag = false;
	trip_flag = plan_flag = new_plan_flag = new_plan_skim_flag = problem_flag = cap_const_flag = fuel_const_flag = choice_flag = false;

	trip_file = 0;
	plan_file = new_plan_file = 0;
	new_plan_skim_file = 0;
	problem_file = 0;
...
void Converge_Service::Converge_Service_Keys (int *keys)
{
	Control_Key control_keys [] = { //--- code, key, level, status, type, default, range, help ----
		...
		{ PARKING_CAPACITY_CRITERIA, "PARKING_CAPACITY_CRITERIA", LEVEL0, OPT_KEY, FLOAT_KEY, "0.0", "0..10.0", NO_HELP },
