...
void Converge_Service::Program_Control (void)
{
	...
	if (plan_memory_flag) {
		time_output_flag = (sort_type == TIME_SORT);
		Trip_Sort (TRAVELER_SORT);
		Time_Sort_Flag (false);

		if (trip_flag && !plan_flag) {
			trip_memory_flag = true;
		}
		if (trip_memory_flag) {
			System_Data_Reserve (TRIP, 0);
		}
	...
	Rider_Flag (System_File_Flag (NEW_RIDERSHIP) || (System_File_Flag (RIDERSHIP) && update_flag));
	...
	if (System_File_Flag (NEW_PLAN)) {
		...
		if (sort_type > 0) {
			new_plan_file->Sort_Type ((Trip_Sort_Type) sort_type);
		} else if (trip_flag) {
	...
	if (System_File_Flag (NEW_PLAN_SKIM)) {
		new_plan_skim_flag = true;
		new_plan_skim_file = System_Plan_Skim_File (true);
	}
	...
	if (max_iteration > 1) {
		...
		if (System_File_Flag (PARK_DEMAND) || System_File_Flag (NEW_PARK_DEMAND)) {
			parking_gap = Get_Control_Double (PARKING_CAPACITY_CRITERIA);
	...
	if (!key.empty ()) {
		...
		for (veh_type_itr = veh_type_array.begin (); veh_type_itr != veh_type_array.end (); veh_type_itr++) {
			if (veh_type_itr->Fuel_Func () > 0 && !functions.Function_Check (veh_type_itr->Fuel_Func ())) {
				Warning (String ("Fuel Function % for Vehicle Type %s was Not Found") % veh_type_itr->Fuel_Func () % veh_type_itr->Type ());
			}
		}
