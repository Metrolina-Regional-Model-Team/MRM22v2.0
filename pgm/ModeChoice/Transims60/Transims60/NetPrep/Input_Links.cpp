...
void NetPrep::Input_Links (void)
{
	...
	if (file->Model_Format () == TPPLUS) {
		...
		field = file->Optional_Field ("SPEED_AB", "SPEEDLMTB", "SPD_AB", "SPEEDAB", "SPDAB");
		if (field < 0) {
			field = file->Optional_Field ("AB_SPD", "AB_SPEED", "ABSPEED", "ABSPD", "MAXSPDAB");
			if (field < 0) {
				field = file->Optional_Field ("SPEED", "SPEEDLMT", "SPD", "MAXSPD", "MAX_SPEED");
			}
		}
		if (field >= 0) {
			file->Set_Units (field, ((units_flag) ? KPH : MPH));
		}
		field = file->Optional_Field ("SPEED_BA", "SPEEDLMTA", "SPD_BA", "SPEEDBA", "SPDBA");
		if (field < 0) {
			field = file->Optional_Field ("BA_SPD", "BA_SPEED", "BASPEED", "BASPD", "MAXSPDBA");
		}
		if (field >= 0) {
			file->Set_Units (field, ((units_flag) ? KPH : MPH));
		}
		field = file->Optional_Field ("FSPD_AB", "FREESPDB", "FREESPD_AB", "FSPDAB", "FREE_AB");
		if (field < 0) {
			field = file->Optional_Field ("AB_FSPD", "AB_FREESPD", "ABFSPD", "ABFSPEED");
			if (field < 0) {
				field = file->Optional_Field ("FREESPD", "FREESPEED", "BASESPD", "FSPEED");
			}
		}
		if (field >= 0) {
			file->Set_Units (field, ((units_flag) ? KPH : MPH));
		}
		field = file->Optional_Field ("FSPD_BA", "FREESPDA", "FREESPD_BA", "FSPDBA", "FREE_BA");
		if (field < 0) {
			field = file->Optional_Field ("BA_FSPD", "BA_FREESPD", "BAFSPD", "BAFSPEED");
		}
		if (field >= 0) {
			file->Set_Units (field, ((units_flag) ? KPH : MPH));
		}
		...
		if (ab_itr == ab_ptr->end ()) {
			...
			if (lanes > 0) {
				...
				if (dir_rec.Speed () < min_spd && spd < min_spd) {
					...
					dir_rec.Time0 ((double) link_rec.Length () / spd + 0.04);
				}
			...
			if (dir_rec.Speed () < min_spd && spd < min_spd) {
				...
				dir_rec.Time0 ((double) link_ptr->Length () / spd + 0.04);
			}
