...
void NetPrep::Program_Control (void)
{
	...
	projection.Set_Projection (proj_service);

	Projection_Data data = proj_service.Data_Projection ();

	if (data.Type () != NO_PROJECTION && 
		((data.Units () != METERS && Metric_Flag ()) || 
		(data.Units () != FEET && !Metric_Flag ()))) {
	
		Error ("Projection Data and Units of Measure are Incompatible");
	}
	...
	if (key.empty ()) {
		...
		if (!System_File_Flag (NEW_LINK)) {
			Error ("A New Link File is Required");
		}
		...
		if (link_shape_flag) {
			...
			link_shape_file.Set_Projection (proj_service);
		...
		key = Get_Control_String (INPUT_LINK_SCRIPT);

		if (!key.empty ()) {
			script_file.File_Type ("Input Link Script");
	...
	if (key.empty ()) {
		...
		if (node_shape_flag) {
			...
			node_shape_file.Set_Projection (proj_service);
	...
	if (key.empty ()) {
		...
		if (zone_shape_flag) {
			...
			zone_shape_file.Set_Projection (proj_service);
	...
	if (!key.empty ()) {
		if (convert_flag) {
			Error ("An Input Link Script was Provided");
		} else {
