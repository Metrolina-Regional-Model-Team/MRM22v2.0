...
void Control_Service::Process_Control_String (String control) 
{
	int i, code;
	bool found;
	String key, nest, buffer, variable;
	string::iterator itr;
	Control_Key_Itr key_itr;
	...
	for (itr = control.begin (); itr != control.end (); itr++) {
		if (*itr == '@') {
			...
			if (found) {
				found = false;
				map_itr = variable_map.find (variable);

				if (map_itr != variable_map.end ()) {
		...
		if (*itr == '@') {
			...
			if (found) {
				...
				if (variable.size () == key.size ()) {
					map_stat = variable_map.insert (Str_Key_Map_Data (variable, control));
					if (!map_stat.second) {
...
//-----------------------------------------------------------
//	Get_Control_List_Groups - search for Bool list parameters 
//-----------------------------------------------------------

bool Control_Service::Get_Control_List_Groups (int code, Bool_List &list)
{
	bool flag = false;
	int num = Highest_Control_Group (code, 0);
...
bool Control_Service::Transit_Range_Key (int code, bool *modes, int nest1, int nest2, int nest3, int nest4)
{
	...
	if (Control_Key_Status (code)) {
		...
		if (!key.empty () && !key.Equals ("ALL") && !key.Equals ("NONE")) {
