...
void Arcview_Base::Arc_Setup (Db_Header *_file)
{
	...
	shape_header.box.xmin = shape_header.box.ymin = -_NAN;
	shape_header.box.xmax = shape_header.box.ymax = _NAN;
	
	if (Z_Flag ()) {
		shape_header.zbox.min = shape_header.mbox.min = -_NAN;
		shape_header.zbox.max = shape_header.mbox.max = _NAN;
	}
...
bool Arcview_Base::Arc_Open (string filename)
{
	...
	Convert_In (&(extent.xmin), &(extent.ymin));
	Convert_In (&(extent.xmax), &(extent.ymax));
...
bool Arcview_Base::Arc_Read (int number)
{
	...
	switch ((type % 10)) {
		...
		case POLYGON_TYPE:		//---- polygon ----
			...
			range.xmin = shape.box.xmin;
			range.ymin = shape.box.ymin;
			range.xmax = shape.box.xmax;
			range.ymax = shape.box.ymax;

			Convert_In (&(range.xmin), &(range.ymin));
			Convert_In (&(range.xmax), &(range.ymax));
			...
			if (Z_Flag ()) {
				...
				if (_range.min <= _NAN) _range.min = 0;
				if (_range.max <= _NAN) _range.max = 0;
...
bool Arcview_Base::Set_Projection (Projection_Service &service)
{
	if (!Projection::Set_Projection (service)) return (false);

	M_Flag (service.M_Flag ());
	Z_Flag (service.Z_Flag ());
