...
bool Db_Matrix::Num_Zones (int num)
{
	if (num <= 0)  return (false);
	if (data_flag && num_org == num && num_des == num) return (true);

	Clear_Data (); 
    num_org = num_des = num;

	Allocate_Data (matrix_flag);

	for (int i = 1; i <= num; i++) {
		if (!org_map.insert (Int_Map_Data (i, i - 1)).second) return (false);
		if (!des_map.insert (Int_Map_Data (i, i - 1)).second) return (false);
	}
	return (true);
}
