...
void TripSum::Trip_Processing::operator()()
{
	...
	if (thread_flag) {
		...
		if (exe->loc_share_flag) {
			productions.assign (exe->location_array.size (), 0);
			attractions.assign (exe->location_array.size (), 0);
		}
		...
		end_ptr = &end_time;
		mid_ptr = &mid_time;
		prod_ptr = &productions;
		attr_ptr = &attractions;
		...
		prod_ptr = &exe->productions;
		attr_ptr = &exe->attractions;
	...
	if (thread_flag) {
		...
		if (exe->loc_share_flag) {
			for (i = 0, itr = productions.begin (); itr != productions.end (); itr++, i++) {
				exe->productions [i] += *itr;
			}
			for (i = 0, itr = attractions.begin (); itr != attractions.end (); itr++, i++) {
				exe->attractions [i] += *itr;
			}
		}
