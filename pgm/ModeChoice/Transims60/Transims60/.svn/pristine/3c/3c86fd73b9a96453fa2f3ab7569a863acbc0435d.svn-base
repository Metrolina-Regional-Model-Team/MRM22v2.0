...
void Projection_Service::Add_Keys (void)
{
	...
	Control_Key projection_keys [] = { //--- code, key, level, status, type, default, range, help ----
		...
		{ DATA_COORDINATE_SYSTEM, "DATA_COORDINATE_SYSTEM", LEVEL0, OPT_KEY, LIST_KEY, "", system_range, NO_HELP },
		{ DATA_COORDINATE_ADJUSTMENT, "DATA_COORDINATE_ADJUSTMENT", LEVEL0, OPT_KEY, LIST_KEY, "", adjust_range, NO_HELP },
...
bool Projection_Service::Read_Control (void)
{
	...
	if (exe->Check_Control_Key (DATA_COORDINATE_SYSTEM)) {
		if (first) {
			exe->Print (1);
			first = false;
		}
		if (!data.Projection_System (exe->Get_Control_Text (DATA_COORDINATE_SYSTEM))) {
			exe->Control_Key_Error (DATA_COORDINATE_SYSTEM);
		}
		defined = true;
	}
	if (exe->Check_Control_Key (DATA_COORDINATE_ADJUSTMENT)) {
		if (first) {
			exe->Print (1);
			first = false;
		}
		if (!data.Projection_Adjustment (exe->Get_Control_Text (DATA_COORDINATE_ADJUSTMENT))) {
			exe->Control_Key_Error (DATA_COORDINATE_ADJUSTMENT);
		}
		defined = true;
	}
