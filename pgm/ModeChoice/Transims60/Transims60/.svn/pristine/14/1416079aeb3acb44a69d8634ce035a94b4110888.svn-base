...
void Data_Service::Program_Control (void)
{
	...
	if (Control_Key_Status (PARKING_DEMAND_TYPE)) {
		Print (1);
		key = Get_Control_Text (PARKING_DEMAND_TYPE);

		if (!key.empty ()) {
			park_demand_array.Demand_Type (Park_Demand_Code (key));
		}
	}

	//---- parking penalty function ----

	if (Control_Key_Status (PARKING_PENALTY_FUNCTION)) {
		if (!Check_Control_Key (PARKING_DEMAND_TYPE)) {
			Print (1);
		}
		key = Get_Control_Text (PARKING_PENALTY_FUNCTION);

		if (!key.empty ()) {
			park_demand_array.Penalty_Function (functions.Max_Function () + 1);

			functions.Add_Function (park_demand_array.Penalty_Function (), key, false);
		}
	}
}
...
void Data_Service::Execute (void)
{
	...
	file = System_File (PARK_DEMAND);
	if (file->flag && file->read) Read_Park_Demand (*(System_Park_Demand_File ()));

	//---- plan skims ----

	file = System_File (PLAN_SKIM);
	if (file->flag && file->read) Read_Plan_Skims (*(System_Plan_Skim_File ()));
