...
{
	int lane, cell, subarea, dir_index, offset, max_move;
	bool loaded, type_flag, bound_flag, side_flag;
	Dtime time, step_size;
...
#ifdef DUMP_DETAILS
	if (sim->dump_flag && traveler == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Load dir=" << dir_index << " type=" << type_flag << " veh=" << sim_travel_ptr->Vehicle ();
#endif
	...
	if (bound_flag) {	//---- external entry point ----

		step_size = sim->method_time_step [sim_dir_ptr->Method ()];
		max_move = DTOI (sim->UnRound (sim_dir_ptr->Speed () * step_size) / sim->param.cell_size);
		...
		if (lane_list.size () > 0) {
			exe->random.Randomize (lane_list);

			for (lane_itr = lane_list.begin (); lane_itr != lane_list.end (); lane_itr++) {
				lane = *lane_itr;
				if (Gap_Check (sim_dir_ptr, max_move, lane, cell) < 0) continue;
	...
	} else if (side_flag) {		//---- load from the right ----
	...
	if (!loaded) {
#ifdef DUMP_DETAILS
		if (sim->dump_flag && traveler == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Load wait=" << sim_travel_ptr->Wait () << " cell=" << cell;
#endif
...
#ifdef DUMP_DETAILS
	if (sim->dump_flag && traveler == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Load dir=" << dir_index << " lane=" << lane << " cell=" << cell;
#endif
	...
	step.Speed ((bound_flag) ? sim_travel_ptr->Speed () : 0);
