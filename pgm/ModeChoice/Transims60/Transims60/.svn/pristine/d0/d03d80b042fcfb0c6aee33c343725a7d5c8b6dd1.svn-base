...
void Simulator::Execute (void)
{
	...
	clock_t start, input_total, update_total, travel_total, output_total, node_total, path_total, diff;
		...
		if (backup_flag) {
			for (time_itr = backup_times.begin (); time_itr != backup_times.end (); time_itr++) {
				if (*time_itr == time_step) {
					...
					sim_plan_array.Pack (file);
					sim_leg_array.Pack (file);
					sim_veh_array.Pack (file);
					sim_veh_map.Pack (file);
...
#ifdef DUMP_DETAILS
		dump_flag = (dump_start <= time_step && dump_start > 0);
#endif
		if (random_time > 0) {
...
//exe->Write (1, " step=") << time_step;
		...
		if (io_flag) {
			...
			output_total += diff = (clock () - start);
//exe->Write (0, " out=") << diff;
			//---- update the network ----

			start = clock ();
			sim_update_step.Start_Processing ();
			update_total += diff = (clock () - start);
//exe->Write (0, " up=") << diff;
			//---- read plans ----
			...
			if (read_status) {
				...
				input_total += diff = (clock () - start);
//exe->Write (0, " read=") << diff;
				if (read_all_flag) {
					End_Progress ();
		...
		travel_total += diff = (clock () - start);
//exe->Write (0, " trv=") << diff;
		//---- process the network traffic ----

		start = clock ();
		sim_node_step.Start_Processing ();
		node_total += diff = (clock () - start);
//exe->Write (0, " node=") << diff;
