...
{
#ifdef CHECK
	if (plan_index >= 0 && (int) sim->sim_plan_array.size () <= plan_index) sim->Error (String ("Sim_Travel_Data::Get_Plan: plan_index=%d vs %d traveler=%d step=%d") % plan_index % sim->sim_plan_array.size () % Traveler () % sim->time_step);
#endif
