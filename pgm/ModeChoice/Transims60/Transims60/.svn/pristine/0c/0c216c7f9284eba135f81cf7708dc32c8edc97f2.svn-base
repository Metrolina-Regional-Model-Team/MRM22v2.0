...
int Equation_Array::Apply_Equation (int number, int time0, double volume, double enter, int capacity, int length, int lanes)
{
	if (time0 < 1) time0 = 1;
	...
	if (volume <= 0 && enter <= 0) {
		...
		if (eq_ptr->type == BPR || eq_ptr->type == BPR_PLUS) {
			...
			cap /= lanes;
			volume /= lanes;
			double density = volume / UnRound (length);
