...
void Simulator_Service::Global_Data (void)
{
	...
	gap_speed.push_back (190);
	gap_speed.push_back (410);
	gap_speed.push_back (620);
	gap_speed.push_back (790);

	if (Metric_Flag ()) {
		for (int_itr = gap_speed.begin (); int_itr != gap_speed.end (); int_itr++) {
			*int_itr = DTOI (*int_itr / METERTOFEET);
		}
	}
