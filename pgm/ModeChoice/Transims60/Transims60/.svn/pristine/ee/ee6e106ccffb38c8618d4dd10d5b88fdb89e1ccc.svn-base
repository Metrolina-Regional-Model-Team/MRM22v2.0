...
int ConvertTrips::Set_Trips (ConvertTrip_Itr group, int org, int des, int trips, int num, int t1, int t2, bool return_flag)
{
	...
	bool org_flag, des_flag, script_flag, skip, percent_flag;
...
retry:
		...
		if (group->Function () > 0) {
			percent = random_duration.Probability () * 100.0;
			duration.Hours (functions.Apply_Function (group->Function (), percent));

			if (duration < 0) {
				Warning ("Duration Function generated Negative Value");
				duration = group->Duration ();
			}
		}
		...
		if (group->Return_Mode () == 0) {
			if (mode == PNR_OUT_MODE) mode = PNR_IN_MODE;
			if (mode == KNR_OUT_MODE) mode = KNR_IN_MODE;
		} else {
			mode = group->Return_Mode ();
		}
		if (group->Return_Purpose () != 0) {
			purpose = group->Return_Purpose ();
		}
