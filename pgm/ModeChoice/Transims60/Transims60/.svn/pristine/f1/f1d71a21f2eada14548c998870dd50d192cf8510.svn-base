...
{
	int index, length, offset, diff, lot, org, imped, ttime, cost, rate, acc, penalty;
	...
	from_ptr->clear ();
	park_flag = park_cap_flag = walk_acc_flag = access_flag = false;
	...
	for (org=0, org_itr = org_ptr->begin (); org_itr != org_ptr->end (); org_itr++, org++) {
		...
		if (exe->access_link_flag) {
			...
			for (; acc >= 0; acc = acc_ptr->Next (ab_flag)) {
				...
				if (path_param.mode == KNR_IN_MODE || path_param.mode == KNR_OUT_MODE) {
					cost = 0;
				} else {
					cost = Round (cost);
				}
				...
				if (!forward_flag && path_param.park_pen_flag) {
					penalty = exe->park_demand_array.Penalty (lot, path_data.Time ());
...
{
	int index, length, offset, diff, lot, des, imped, ttime, cost, rate, acc, low_imp, hi_imp, tot_imp, tod, penalty;
	bool link_flag, ab_flag;
	...
	for (des=0, des_itr = des_ptr->begin (); des_itr != des_ptr->end (); des_itr++, des++) {
		...
		if (exe->access_link_flag) {
			...
			for (; acc >= 0; acc = acc_ptr->Next (ab_flag)) {
				...
				if (forward_flag && path_param.park_pen_flag) {
					penalty = exe->park_demand_array.Penalty (lot, tod);
					if (penalty >= MAX_PENALTY) {
						park_cap_flag = true;
						continue;
					}
