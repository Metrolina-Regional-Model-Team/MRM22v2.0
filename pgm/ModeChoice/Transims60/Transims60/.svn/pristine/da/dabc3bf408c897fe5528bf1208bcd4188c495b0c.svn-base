...
{
	int period, num_periods, code, prev_purp, increment, node, zone, org, des;
	...
	zone_flag = exe->System_Data_Flag (ZONE);
	...
	while (file->Read_Trip (trip_data)) {
		...
		if (exe->select_org_zones) {
			...
			if (map_itr != exe->location_map.end ()) {
				...
				if (zone_flag) {
					zone = exe->zone_array [location_ptr->Zone ()].Zone ();
				} else {
					zone = location_ptr->Zone ();
				}
				if (!exe->org_zone_range.In_Range (zone)) continue;
			}
		}
		if (exe->select_des_zones) {
			...
			if (map_itr != exe->location_map.end ()) {
				...
				if (!exe->des_zone_range.In_Range (zone)) continue;
			}
		}
		...
		//---- summarize mode purpose groups ----

		if (exe->group_file_flag || exe->group_report_flag) {
			if (trip_data.Purpose () < 0 || trip_data.Purpose () >= 100) {
				exe->Warning (String ("Trip Purpose %d is Out of Range") % trip_data.Purpose ());
			}
			org = des = 0;
			map_itr = exe->location_map.find (trip_data.Origin ());
			if (map_itr != exe->location_map.end ()) {
			...
			if (org > 0 && des > 0) {
				org = exe->zone_equiv.Zone_Group (org);
				des = exe->zone_equiv.Zone_Group (des);

				if (org > 0) {
					exe->mode_purp_groups [org] [trip_data.Mode ()] [trip_data.Purpose ()] [0]++;
				}
				if (des > 0) {
					exe->mode_purp_groups [des] [trip_data.Mode ()] [trip_data.Purpose ()] [1]++;
				}
			}
		}
