...
bool In_Extents (Points &points, XY_Range &range)
{
	double xmin1, ymin1, xmax1, ymax1;
	...
	xmin1 = ymin1 = MAX_INTEGER;
	xmax1 = ymax1 = -MAX_INTEGER;
	...
	return (xmin1 <= range.xmax && xmax1 >= range.xmin && ymin1 <= range.ymax && ymax1 >= range.ymin);
}

bool In_Extents (XY_Range &box, XY_Range &range)
{
	return (box.xmin <= range.xmax && box.xmax >= range.xmin && box.ymin <= range.ymax && box.ymax >= range.ymin);
}

bool In_Extents (XYZ_Point &pt, XY_Range &range)
{
...
XY_Range Buffer_Box (XY_Range box, double buffer) 
{
	box.xmin -= buffer;
	box.ymin -= buffer;
	box.xmax += buffer;
...
XY_Range Buffer_Box (XYZ_Point pt, double buffer) 
{
	XY_Range box;

	box.xmin = pt.x - buffer;
	box.ymin = pt.y - buffer;
	box.xmax = pt.x + buffer;
	box.ymax = pt.y + buffer;
...
double Point_Shape_Distance (XYZ_Point pt, Points &points)
{
	int p;
	...
	for (p = 0, pt_itr = points.begin (); pt_itr != points.end (); pt_itr++, p++) {
		if (p > 0) {
			if (pt_itr->x < x1) {
				if (pt_itr->x <= pt.x && pt.x <= x1) {
					dx = (pt.x - pt_itr->x) / (x1 - pt_itr->x);
					y2 = pt_itr->y + dx * (y1 - pt_itr->y);
					diff = abs (pt.y - y2);
					if (diff < best_diff) {
						best_diff = diff;
					}
			...
			if (pt_itr->y < y1) {
				if (pt_itr->y <= pt.y && pt.y <= y1) {
					dy = (pt.y - pt_itr->y) / (y1 - pt_itr->y);
					x2 = pt_itr->x + dy * (x1 - pt_itr->x);
					diff = abs (pt.x - x2);
					if (diff < best_diff) {
						best_diff = diff;
					}
...
double Shape_Length (Points &points)
{
