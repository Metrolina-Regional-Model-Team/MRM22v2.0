...
int Path_Builder::Trace_Path (Trip_End *org, Path_End_Array *from, Path_End *to)
{
	...
	int imped, len, cost, mode, path, prev_len, prev_imp, prev_cost, wait_imp, wait_cost;
	int from_index, from_type, from_path, from_dir, to_index, to_type, to_path, to_dir;
	bool ab_flag, pnr_flag;
	...
	pnr_flag = (mode == PNR_OUT_MODE || mode == PNR_IN_MODE || mode == KNR_OUT_MODE || mode == KNR_IN_MODE);
...
next_mode:
	...
	for (++path_itr; path_itr != to->end (); path_itr++) {
		...
		imped = Resolve (to_ptr->Imped_Diff (from_ptr->Imped ()));
	...
	for (; from_type != FROM_ID; to_ptr = from_ptr) {
		...
		if (from_type == DIR_ID) {
			...
			if (to_index < 0 || to_index >= (int) wait_path [from_path].size ()) exe->Error ("Path_Builder::Trace: wait_path to_index");
#endif
		...
		if (!path_param.skim_only || (flow_flag && mode == DRIVE_MODE)) {
			if (forward_flag) {
				...
				if (to_type == STOP_ID && mode == WALK_MODE) {
					Add_Leg (TRANSIT_MODE, to_type, to_index, 0, prev_imp, prev_time, prev_len, prev_cost);
				}
			...
			if (type == DIR_ID) {
				...
				if (mode == DRIVE_MODE) {
					...
					if (!forward_flag) {
						if (from_end->End_Type () == PARKING_ID) {
							...
							if (path_param.mode == PNR_OUT_MODE || path_param.mode == KNR_OUT_MODE) {
								parking_end = &parking_lots [from_end->Trip_End ()];
								if (parking_end->Best () < 0) return (0);

								to_ptr = &to_parking [parking_end->Best ()][0];

								time = to_ptr->Time () - from_ptr->Time ();
								imped = Resolve (from_ptr->Imped_Diff (to_ptr->Imped ()));
								cost = from_ptr->Cost () - to_ptr->Cost ();

								Add_Leg (OTHER_MODE, PARKING_ID, from_ptr->From (), from_ptr->Dir (), imped, time, 0, cost);
							}
							continue;
						}
					}
				...
				if (from_type == NODE_ID) {
					...
					if (itr != exe->ab_link.end ()) {
						...
						exe->Error (String ("Node Path Error %d-%d, Mode=%s") % exe->node_array [anode].Node () % exe->node_array [bnode].Node () % Mode_Code ((Mode_Type) mode));
					}
				} else if (from_type == FROM_ID) {
	...
	for (++path_ritr; path_ritr != from_end->rend (); path_ritr++) {
		...
		if (from_end->End_Type () != PARKING_ID || from_ptr->Type () != PARKING_ID || forward_flag || (path_param.mode != PNR_IN_MODE && path_param.mode != KNR_IN_MODE)) {
	...
	if (from_end->End_Type () == PARKING_ID) {
		...
		path_itr = to->begin ();
		if (to_ptr->Type () == PARKING_ID) {
			if (path_param.mode == PNR_OUT_MODE || path_param.mode == KNR_OUT_MODE) goto next_mode;
			if (!forward_flag && (path_param.mode == PNR_IN_MODE || path_param.mode == KNR_IN_MODE)) goto next_mode;
		}
		if (forward_flag) {
		...
		imped = Resolve (to_ptr->Imped_Diff (path_itr->Imped ()));
