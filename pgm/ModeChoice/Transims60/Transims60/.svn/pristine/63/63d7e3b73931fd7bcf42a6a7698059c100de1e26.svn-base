...
void TripPrep::Program_Control (void)
{
	...
	if (!key.empty ()) {
		...
		script_file = new Trip_File (MODIFY, BINARY);
		script_file->Create_Fields ();

		org_fld = script_file->Add_Field ("ORG_ZONE", DB_INTEGER, 4, NO_UNITS, true);
		des_fld = script_file->Add_Field ("DES_ZONE", DB_INTEGER, 4, NO_UNITS, true);
	}
