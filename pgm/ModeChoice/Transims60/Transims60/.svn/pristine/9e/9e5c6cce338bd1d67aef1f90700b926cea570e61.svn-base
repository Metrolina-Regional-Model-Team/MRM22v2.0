...
void ArcPerf::Program_Control (void)
{
	...
	Str_Itr str_itr;
	double dvalue;
	bool binary, ignore_flag, perf_in, turn_in;
	...
	if (!key.empty ()) {
		...
		arcview_link_data.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_link_dir.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_perf.Set_Projection (projection);
	...
	for (n=1; n <= num; n++) {
		...
		image_rec.file->Set_Projection (projection);
