...
bool Sim_Node_Process::Cell_Use (Sim_Dir_Ptr sim_dir_ptr, int lane, int cell, Travel_Step &step, bool use_flag)
{
	...
	if (!step.New_Link () && (cell <= sim_dir_ptr->In_Cell () || cell >= sim_dir_ptr->Out_Cell ())) return (true);
	...
	if (!Use_Permission (sim_dir_ptr->Use (), use)) goto use_problem;

	if (veh_type >= 0 && sim_dir_ptr->Min_Veh_Type () >= 0) {
		if (sim_dir_ptr->Use_Type () == PROHIBIT) {
			if (veh_type >= sim_dir_ptr->Min_Veh_Type () && veh_type <= sim_dir_ptr->Max_Veh_Type ()) goto use_problem;
		} else {
			if (veh_type < sim_dir_ptr->Min_Veh_Type () || veh_type > sim_dir_ptr->Max_Veh_Type ()) goto use_problem;
		}
	}
	if (type > 0 && sim_dir_ptr->Min_Traveler () > 0) {
		if (sim_dir_ptr->Use_Type () == PROHIBIT) {
			if (type >= sim_dir_ptr->Min_Traveler () && type <= sim_dir_ptr->Max_Traveler ()) goto use_problem;
		} else {
			if (type < sim_dir_ptr->Min_Traveler () || type > sim_dir_ptr->Max_Traveler ()) goto use_problem;
		}
	}
	step.sim_travel_ptr->Problem (0);
...
use_problem:
	if (!step.New_Link ()) return (true);
	step.sim_travel_ptr->Problem (USE_PROBLEM);
	return (false);
}
