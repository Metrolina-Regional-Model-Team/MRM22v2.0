...
typedef struct {
	...
	void Clear (void) {
		num_reskim = total_records = num_reroute = num_update = num_copied = num_build = select_records = select_weight = 0;
	}
} Counters;
...
protected:
	enum Converge_Service_Keys { 
		...
		LINK_CONVERGENCE_CRITERIA, TRIP_CONVERGENCE_CRITERIA, TRANSIT_CAPACITY_CRITERIA, PARKING_CAPACITY_CRITERIA,
	...
	bool trip_flag, plan_flag, plan_skim_flag, trip_set_flag, trip_memory_flag, plan_memory_flag, sched_acc_flag, time_output_flag;
	bool new_plan_flag, new_plan_skim_flag, problem_flag, new_set_flag, problem_set_flag, plan_set_flag, preload_flag, time_order_flag;
	bool flow_flag, turn_flag, reroute_flag, cap_const_flag, fuel_const_flag, fuel_flag, choice_flag, info_flag, set_mode_flag;

	Dtime cap_duration, fuel_duration;
	int iteration, max_iteration, max_speed_updates, num_trip_sel, num_trip_rec;
	int num_file_sets, num_time_updates, num_trip_parts, potential;
	int minimum_fuel, seek_fuel, set_mode, set_veh_type;
	double initial_factor, factor_increment, maximum_factor;
	double link_gap, trip_gap, transit_gap, parking_gap, min_speed_diff, percent_selected, total_percent;
	...
	Plan_Skim_File *plan_skim_file, *new_plan_skim_file;
	...
	void Set_Mode (int mode)                           { set_mode = mode; set_mode_flag = (mode > 0); }
	void Set_Veh_Type (int veh_type)                   { set_veh_type = veh_type; }
	...
	public:
		...
		Counters * Counter_Ptr (int part = 0);
...
private:
	...
	bool   Input_Trips (void);
	bool   Select_Trips (void);
