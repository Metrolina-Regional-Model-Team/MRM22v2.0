...
{
	int i, j, n, mode, nest, group, table, num;
	...
	for (i = 0; i < num_modes; i++) {
		...
		if (value > 0.0) {
			...
			if (key_mode >= 0 && i == key_mode) return (0);
	...
	if (plan_flag) {
		...
		if (sum > 0.0) {
			...
			for (i = 0; i < num_modes; i++) {
				...
				if (level <= sum) {
					num = mode_plan [i];
					int_itr = plan_num_map.find (num);

					trips = plan_skims [int_itr->second]->Num_Legs ();
					...
					if (summary_flag) {
						market_seg [0] [i] [model] += trips;
						market_seg [0] [i] [num_models] += trips;
						market_seg [0] [num_modes] [model] += trips;
						market_seg [0] [num_modes] [num_models] += trips;
					}
					if (market_flag && segment > 0) {
						market_seg [segment] [i] [model] += trips;
						market_seg [segment] [i] [num_models] += trips;
						market_seg [segment] [num_modes] [model] += trips;
						market_seg [segment] [num_modes] [num_models] += trips;
					}
					if (prod_sum_flag) {
						prod_share [i] [o] += trips;
						prod_share [num_modes] [o] += trips;
					}
					if (attr_sum_flag) {
						attr_share [i] [d] += trips;
						attr_share [num_modes] [d] += trips;
					}
					...
					for (n = num_nests - 1; n >= 0; n--) {
						...
						if (nest >= 0) {
							if (summary_flag) {
								market_seg [0] [nest] [model] += trips;
								market_seg [0] [nest] [num_models] += trips;
							}
							if (market_flag && segment > 0) {
								market_seg [segment] [nest] [model] += trips;
								market_seg [segment] [nest] [num_models] += trips;
							}
							if (prod_sum_flag) {
								prod_share [nest] [o] += trips;
							}
							if (attr_sum_flag) {
								attr_share [nest] [d] += trips;
							}
							mode = nest;
						}
					}
