...
bool VissimNet::Speed_Fields (void)
{
	...
	if (speed_flag) {
		if (pair_itr->first.Equals ("<speedDistrDatPts")) {
			if (speed_pts_flag) Warning ("Speed Points Block was Not Terminated");
			speed_pts_flag = true;
			fx = fy = value = 0.0;
		} else if (pair_itr->first.Equals ("</speedDistrDatPts")) {
			if (!speed_pts_flag) Warning ("Speed Points Block was Not Initialized");
		...
		if (speed_pts_flag) {
			if (pair_itr->first.Equals ("<speedDistributionDataPoint")) {
				...
				if (!speed_pt_flag) Warning ("Speed Point Block was Not Initialized");
				speed_pt_flag = false;
			}
			if (speed_pt_flag) {
				if (pair_itr->first.Equals ("<speedDistributionDataPoint")) {
					speed_pt_flag = !pair_itr->second.Equals ("/>");
