...
void Router::Program_Control (void)
{
	...
	Update_Flag (Set_Control_Flag (UPDATE_PLAN_RECORDS));
	...
	if (new_plan_flag && plan_flag) {
		...
		update_plan_flag = Get_Control_Flag (UPDATE_PLAN_RECORDS);

		reroute_time = Get_Control_Time (REROUTE_FROM_TIME_POINT);
		reroute_flag = (reroute_time > 0);

		Reroute_Time (reroute_time);
		Update_Flag (update_plan_flag);

		if (update_plan_flag && reroute_flag) {
			Warning ("Re-Routing and Updating Plans are Mutually Exclusive");
			update_plan_flag = false;
		}
	} else if (Set_Control_Flag (UPDATE_PLAN_RECORDS)) {
		Error ("Update Plan Records Requires an Input and Output Plan File");
	} else if (Get_Control_Time (REROUTE_FROM_TIME_POINT) > 0) {
		Error ("Re-Routing Requires an Input and Output Plan File");
	}
	...
	if (!key.empty ()) {
		...
		key = Get_Control_Text (ROUTE_WITH_VEHICLE_TYPE);

		if (!key.empty ()) {
			Set_Veh_Type (key.Integer ());
		}
	}
#ifdef DUMP_DETAILS
