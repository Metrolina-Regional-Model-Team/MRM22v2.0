...
void RiderSum::Program_Control (void)
{
	...
	//---- open load summary file ----

	key = Get_Control_String (NEW_LOAD_SUMMARY_FILE);

	if (!key.empty ()) {
		Print (1);
		load_summary_flag = true;

		load_summary_file.File_Type ("New Load Summary File");
		load_summary_file.File_ID ("Detail");

		load_summary_file.Create (Project_Filename (key));
	}

	link_load_flag = System_File_Flag (NEW_PERFORMANCE);
