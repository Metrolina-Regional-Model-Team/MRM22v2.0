...
{
	int i, j, num_legs, stop, stops, link_dir, dir_index, dir, overlap, runs, count, index;
	int route_field, stops_field, mode_field, type_field, name_field, notes_field;
	double offset, length, stop_off, link_len, side, side_offset, speed, tot_len;
	bool offset_flag;
	Dtime time;
	...
	for (line_itr = line_array.begin (); line_itr != line_array.end (); line_itr++) {
		...
		if (driver_flag) {
			...
			if (count > 0) {
				Warning (String ("Route %d %s has %d links (%.1lf miles) beyond the last stop") % line_itr->Route () % line_itr->Name () % count % External_Units (length, MILES));
			}
			...
			arcview_route.Put_Field ("LENGTH", tot_len);
		...
		if (schedule_flag) {
			...
			for (i = j = 0; i < transit_time_periods.Num_Ranges (); i++) {
				runs = 0;
				time = 0;
				...
				if (driver_flag) {
					if (time > 0) {
						speed = tot_len / time.Seconds ();
					} else {
						speed = 0;
					}
					arcview_route.Put_Field (run_field + j++, speed);
				}
			}
		}
