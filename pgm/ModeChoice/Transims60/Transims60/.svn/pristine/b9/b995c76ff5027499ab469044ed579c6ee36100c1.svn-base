...
double Line_Array::Ridership_Gap (bool cap_flag, double factor, bool zero_flag)
{
	int r, capacity, cap, max_load, max, penalty, load_func, func;
	Dtime wait;
	double factor1, delay_fac, delay1, delay2, seconds, ratio;
	...
	for (line_itr = begin (); line_itr != end (); line_itr++) {
		if (cap_flag) {
			...
			load_func = veh_type_ptr->Load_Func ();
		}
		for (stop_itr = line_itr->begin (); stop_itr != line_itr->end (); stop_itr++) {
			for (r=0, run_itr = stop_itr->begin (); run_itr != stop_itr->end (); run_itr++, r++) {
				...
				if (cap_flag && (run_itr->Load () > 0 || penalty > 0)) {
					...
					if (run_itr->Load () > cap && cap > 0) {
						...
						if (func > 0) {
							ratio = (double) run_itr->Load () / cap;
							seconds = exe->Scale (exe->functions.Apply_Function (func, ratio));
							
							if (seconds > MAX_PENALTY) {
								wait = MAX_PENALTY;
							} else if (seconds > (double) wait) {
								wait = exe->DTOI (seconds);
							}
						}
					} else {
						wait = 0;
