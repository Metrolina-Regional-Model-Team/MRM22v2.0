...
protected:
	enum TripSum_Keys { 
		...
		NEW_MODE_PURPOSE_GROUP_FILE, NEW_MODE_PURPOSE_GROUP_FORMAT, NEW_LOCATION_SHARE_FILE, NEW_LOCATION_SHARE_FORMAT,
...
private:
	enum TripSum_Reports { TOP_100_LINKS = 1, TOP_100_LANES, TOP_100_VC_RATIOS, ZONE_EQUIV, TIME_EQUIV, 
		TRIP_DIURNAL, TRIP_TIME, TRIP_LENGTH, TRIP_PURPOSE, MODE_LENGTH, MODE_PURPOSE, TRIP_GROUPS };
	...
	bool group_report_flag, group_file_flag, loc_share_flag;
	
	Db_File diurnal_file, trip_time_file, trip_len_file;
	Db_Header link_trip_file, loc_trip_file, zone_trip_file, group_file, loc_share_file;
	Db_Matrix *trip_table_file;

	Integers start_time, mid_time, end_time, productions, attractions;
	...
	void Write_Mode_Purpose_Groups (void);
	void Write_Location_Shares (void);
	...
	void Mode_Purpose_Group_Report ();
	void Mode_Purpose_Group_Header ();
	...
	private:
		...
		Integers    productions, attractions, *prod_ptr, *attr_ptr;
