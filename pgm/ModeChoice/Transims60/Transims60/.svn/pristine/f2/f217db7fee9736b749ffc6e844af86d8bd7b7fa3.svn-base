...
{
	int lvalue, index, type, link;
	Int_Map_Itr map_itr;

	bool stop_flag = System_Data_Flag (TRANSIT_STOP);

	link = file.Link ();
	if (link <= 0) link = (int) access_array.size () + 1;

	access_rec.Link (link);
	...
	if (index < 0) {
		if (file.Version () > 40 || !(type == LOCATION_ID && access_rec.To_Type () == STOP_ID)) {
			Warning (String ("Access Link %d From_ID %d was Not Found in the %s file") % link % lvalue % ID_Code ((ID_Type) type));
		}
		return (false);
	}
	...
	if (index < 0) {
		if (file.Version () > 40 || !(type == LOCATION_ID && access_rec.From_Type () == STOP_ID)) {
			Warning (String ("Access Link %d To_ID %d was Not Found in the %s file") % link % lvalue % ID_Code ((ID_Type) type));
		}
		return (false);
	}
