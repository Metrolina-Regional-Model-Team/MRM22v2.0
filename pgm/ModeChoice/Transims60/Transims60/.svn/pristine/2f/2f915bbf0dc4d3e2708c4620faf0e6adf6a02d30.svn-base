...
bool Converge_Service::Save_Plans (Plan_Ptr_Array *array_ptr, int part)
{
	...
	Counters *counter_ptr = part_processor.Counter_Ptr (part);
	...
	for (itr = array_ptr->begin (); itr != array_ptr->end (); itr++) {
		...
		if (plan_memory_flag) {
			...
			} else if (new_ptr->Problem () == PARK_CAP_PROBLEM) {
				priority = CRITICAL;
			}
			if (keep_new) {
				plan_ptr->clear ();
...
select_plans:
			if (plan_ptr->Priority () != SKIP) {
				if (iteration > 1 && plan_ptr->Priority () == CRITICAL && priority == CRITICAL && new_ptr->Problem () != PARK_CAP_PROBLEM) {
				...
				if (select_priorities && select_priority [plan_ptr->Priority ()]) {
					counter_ptr->select_records++;
					counter_ptr->select_weight += plan_ptr->Priority ();
				}
			}
			...
			if (new_ptr->Problem () == 0) {
				if (new_plan_flag || new_plan_skim_flag) {
					if (new_ptr->External_IDs ()) {
						...
						if (new_plan_skim_flag) {
							new_plan_skim_file->Put_Data (*new_ptr);
							new_plan_skim_file->Write ();
						}
					}
				}
