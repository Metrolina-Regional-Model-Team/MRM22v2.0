...
void Db_Base::Set_Units (int fld, Units_Type units)
{
	if (fld >= 0) {
...
//exe->Write (1, " copy format=") << input.Dbase_Format () << " to " << Dbase_Format ();
...
//exe->Write (1, " name=") << in_fld->Name () << " size=" << in_fld->Size () << " lvalue=" << lvalue;
						if (!Write_Field (out_fld, (void *) &lvalue, DB_INTEGER)) return (false);
//exe->Write (0, " out size=") << out_fld->Size () << " value=" << Get_Integer (in_fld->Name ());
						break;
					case DB_DOUBLE:
					case DB_FIXED:
					case DB_FLOAT:
