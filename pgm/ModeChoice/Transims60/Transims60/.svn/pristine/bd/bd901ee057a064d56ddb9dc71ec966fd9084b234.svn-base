...
void Router_Service::Program_Control (void)
{
	...
	if (!parkride_path_flag && !kissride_path_flag) {
		System_File_False (SHAPE);
	}
	...
	if (transit_path_flag) {
		...
		//--- fare class processing ----

		path_param.fare_class = 0;
		path_param.fare_class_flag = false;

		if (Highest_Control_Group (CASH_FARE_PERCENTS, 0) || Highest_Control_Group (CARD_FARE_PERCENTS, 0) || Highest_Control_Group (SPECIAL_FARE_PERCENTS, 0)) {
			Print (1);
			path_param.fare_class_flag = fare_list_flag = true;

			//---- cash fare percents ----

			Get_Control_List_Groups (CASH_FARE_PERCENTS, cash_percents);

			//---- card fare percents ----

			Get_Control_List_Groups (CARD_FARE_PERCENTS, card_percents);

			//---- special fare percents ----

			Get_Control_List_Groups (SPECIAL_FARE_PERCENTS, special_percents);
		}
		...
		if (parkride_path_flag) {
			Print (1);
			Get_Control_List_Groups (NUM_PARK_RIDE_OPTIONS, num_parkride);
			path_param.num_parkride = num_parkride [0];

			Get_Control_List_Groups (MAX_PARK_RIDE_PERCENTS, max_parkride);
			path_param.max_parkride = max_parkride [0];

			Get_Control_List_Groups (BUS_PARK_RIDE_OPTIONS, num_bus_pnr);
			path_param.num_bus_pnr = num_bus_pnr [0];

			Get_Control_List_Groups (BUS_PARK_RIDE_PERCENTS, max_bus_pnr);
			path_param.max_bus_pnr = max_bus_pnr [0];

			Get_Control_List_Groups (RAIL_PARK_RIDE_OPTIONS, num_rail_pnr);
			path_param.num_rail_pnr = num_rail_pnr [0];

			Get_Control_List_Groups (RAIL_PARK_RIDE_PERCENTS, max_rail_pnr);
			path_param.max_rail_pnr = max_rail_pnr [0];

			Get_Control_List_Groups (MAJOR_PARK_RIDE_OPTIONS, num_major_pnr);
			path_param.num_major_pnr = num_major_pnr [0];

			Get_Control_List_Groups (MAJOR_PARK_RIDE_PERCENTS, max_major_pnr);
			path_param.max_major_pnr = max_major_pnr [0];

			if (path_param.num_parkride == 0 && path_param.num_bus_pnr == 0 && path_param.num_rail_pnr == 0 && path_param.num_major_pnr == 0) {
				Error ("At Least One Type of Park-Ride Options Must be Non-Zero");
			}
		}

		if (parkride_path_flag || kissride_path_flag) {
			Print (1);
			Get_Control_List_Groups (DRIVE_DISTANCE_PENALTIES, drive_penalty);
			path_param.drive_penalty = drive_penalty [0];
		}
		...
		if (kissride_path_flag) {
			...
			Get_Control_List_Groups (NUM_KISS_RIDE_OPTIONS, num_kissride);
			path_param.num_kissride = num_kissride [0];
	...
	if (drive_path_flag || transit_path_flag) {
		...
		if (System_File_Flag (PARK_DEMAND) || System_File_Flag (NEW_PARK_DEMAND)) {
			path_param.park_demand_flag = true;

			if (System_File_Flag (NEW_PARK_DEMAND)) {
				park_demand_flag = true;
			}
	...
	if (transit_path_flag) {
		...
		if (!text.empty ()) {
			...
			path_param.fare_class_flag = fare_class_flag = true;
			i = 0;
