...
#ifdef DUMP_DETAILS
	if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Remove dir=" << step.Dir_Index () << " vehicle=" << step.sim_travel_ptr->Vehicle ();
#endif
...
#ifdef DUMP_DETAILS
	if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Remove parked=" << park_flag << " cells=" << veh_type_ptr->Cells () << " status=" << step.sim_travel_ptr->Status ();
#endif
...
#ifdef DUMP_DETAILS
		if (sim->dump_flag && step.Traveler () == sim->dump_traveler) sim->Write (1, sim->time_step.Time_String ()) << " Remove dir=" << dir_index << " lane=" << lane << " offset=" << sim_veh_ptr->offset << " traveler=" << traveler;
#endif
		if (traveler == step.Traveler ()) {
			sim_dir_ptr->Remove (index);
#ifdef CHECK
		} else if (!park_flag && traveler > 0 && step.sim_travel_ptr->Status () == ON_NET_DRIVE) {
			sim->Error (String ("Simulator_Service::Remove_Vehicle: Traveler %d at %d-%d-%d") % step.Traveler () % dir_index % lane % cell);
#endif
		}
	}
}
