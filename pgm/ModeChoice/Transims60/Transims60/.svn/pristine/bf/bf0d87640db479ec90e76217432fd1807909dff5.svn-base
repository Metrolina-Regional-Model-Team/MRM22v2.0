...
void ConvertTrip_Data::Open_Trips (string &name, string &format)
{
	if (!name.empty ()) {
		...
		exe->Print (0, " (Zones=") << trip_file->Num_Des () << " Tables=" << trip_file->Tables () << ")";
	}
}
...
void ConvertTrip_Data::Return_Period (Dtime tod)
{
	int p;
	Diurnal_Itr itr;

	for (p = 0, itr = diurnal_array.begin (); itr != diurnal_array.end (); itr++, p++) {
		if (tod > itr->Start_Time () && tod < itr->End_Time ()) {
			returns [p]++;
			break;
		}
	}
