...
void PlanSum::Program_Control (void)
{
	...
	key = Get_Control_String (NEW_PLAN_STOP_FILE);

	if (!key.empty ()) {
		plan_stop_file.File_Type ("New Plan Stop File");
		plan_stop_flag = true;

		if (Check_Control_Key (NEW_PLAN_STOP_FORMAT)) {
			plan_stop_file.Dbase_Format (Get_Control_String (NEW_PLAN_STOP_FORMAT));
		}
		plan_stop_file.Add_Field ("HHOLD", DB_INTEGER, 4, NO_UNITS, true);
		plan_stop_file.Add_Field ("PERSON", DB_INTEGER, 2, NO_UNITS, true);
		plan_stop_file.Add_Field ("TOUR", DB_INTEGER, 1, NO_UNITS, true);
		plan_stop_file.Add_Field ("TRIP", DB_INTEGER, 1, NO_UNITS, true);
		plan_stop_file.Add_Field ("START", DB_TIME, TIME_FIELD_SIZE, Time_Format ());

		plan_stop_file.Add_Field ("STOP1", DB_INTEGER, 10);
		plan_stop_file.Add_Field ("STOP2", DB_INTEGER, 10);
		plan_stop_file.Add_Field ("STOP3", DB_INTEGER, 10);
		plan_stop_file.Add_Field ("STOP4", DB_INTEGER, 10);
		plan_stop_file.Add_Field ("STOP5", DB_INTEGER, 10);
		
		plan_stop_file.Add_Field ("MAXLINK", DB_INTEGER, 10);
		plan_stop_file.Add_Field ("MAXLOAD", DB_INTEGER, 10);
		plan_stop_file.Add_Field ("LOADFAC", DB_DOUBLE, 8.2);

		plan_stop_file.Create (Project_Filename (key));

		if (!System_File_Flag (RIDERSHIP)) {
			Error ("An Input Ridership File is Required for Plan Stop Processing");
		} else if (!System_File_Flag (VEHICLE_TYPE)) {
			Error ("A Vehicle Type Fie is Required for Plan Stop Processing");
		} else if (System_File_Flag (NEW_RIDERSHIP)) {
			Error ("An Input and Output Ridership File are Mutually Exclusive");
		}
	}
	...
	if (passenger_flag || transfer_flag || rider_flag || xfer_flag || xfer_detail || on_off_flag || 
		boarding_flag || access_flag || access_group_flag || plan_stop_flag) {
		...
		if ((passenger_flag || rider_flag || plan_stop_flag) && !System_File_Flag (TRANSIT_DRIVER)) {
