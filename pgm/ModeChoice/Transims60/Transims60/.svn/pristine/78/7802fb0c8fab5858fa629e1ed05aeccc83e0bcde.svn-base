...
void ArcNet::Program_Control (void)
{
	...
	if (!key.empty ()) {
		...
		arcview_node.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_zone.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_link.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_center.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_pocket.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_lane_use.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_connect.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_turn.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_sign.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_signal.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_timing.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_phasing.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_detector.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_route.Set_Projection (projection);
		...
		if (driver_flag) {
			...
			arcview_route.Add_Field ("LENGTH", DB_DOUBLE, 8.2, MILES);
		...
		if (schedule_flag) {
			if (transit_time_periods.Num_Periods () > 0) {
				...
				for (i=0; i < transit_time_periods.Num_Ranges (); i++) {
					...
					arcview_route.Add_Field (key, DB_TIME, 7.2, MINUTES);

					if (driver_flag) {
						key = "SPD_" + low.Time_Label ();

						arcview_route.Add_Field (key, DB_DOUBLE, 5.1, MPH);
					}
				}
			} else {
				schedule_flag = false;
	...
	if (!key.empty ()) {
		...
		arcview_driver.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_service.Set_Projection (projection);
	...
	if (!key.empty ()) {
		...
		arcview_route_nodes.Set_Projection (projection);
	...
	if (subzone_flag) {
		...
		if (!key.empty ()) {
			...
			arcview_subzone.Set_Projection (projection);
