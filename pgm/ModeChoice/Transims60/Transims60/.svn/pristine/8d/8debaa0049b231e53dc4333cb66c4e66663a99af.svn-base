...
void Router_Service::Execute (void)
{
	...
	if (transit_path_flag) {
		...
		if (parkride_path_flag) {
			...
			int i, ax, ay, bx, by;
			double factor;
			bool shape_flag = System_File_Flag (SHAPE);
			...
			for (i=0, park_itr = parking_array.begin (); park_itr != parking_array.end (); park_itr++, i++) {
				...
				if (shape_flag) {
					...
					Link_Shape (link_ptr, park_itr->Dir (), points, UnRound (park_itr->Offset ()), 0.0, 0.0);

					if (points.size () > 0) {
						xy_rec.x = Round (points [0].x);
						xy_rec.y = Round (points [0].y);
					} else {
			...
			for (i=0, park_itr = parking_array.begin (); park_itr != parking_array.end (); park_itr++, i++) {
				...
				for (stop_itr = stop_array.begin (); stop_itr != stop_array.end (); stop_itr++) {
					...
					if (shape_flag) {
						...
						Link_Shape (link_ptr, stop_itr->Dir (), points, UnRound (stop_itr->Offset ()), 0.0, 0.0);
						...
						ax += (int) ((bx - ax) * factor + 0.5);
						ay += (int) ((by - ay) * factor + 0.5);
					}
					ax -= xy_rec.x;
					ay -= xy_rec.y;
	...
	//---- intialize park period array ----

	if (!System_File_Flag (PARK_DEMAND)) {
