...
bool Plan_File::Read_Plan (Plan_Data &plan_rec)
{
	...
	if (plan_rec.Depart () < 0) {
		exe->Warning (String ("Plan Record = %d-%d-%d-%d has Negative Departure Time") % 
			plan_rec.Household () % plan_rec.Person () % plan_rec.Tour () % plan_rec.Trip ());
		plan_rec.Depart (0);
	}
...
bool Plan_File::Create_Fields (void) 
{
	...
	Add_Field ("LEG_TIME", DB_TIME, 4, SECONDS, true, NESTED);
	Add_Field ("LEG_LENGTH", DB_UNSIGNED, 4, FEET, true, NESTED);
	Add_Field ("LEG_COST", DB_FIXED, 2.1, CENTS, true, NESTED);
	Add_Field ("LEG_IMPED", DB_UNSIGNED, 4, IMPEDANCE, true, NESTED);
