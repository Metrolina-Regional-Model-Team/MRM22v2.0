	...
	if (num_threads > 1) {
		if (exe->Memory_Flag ()) {
			...
			for (int i=0; i < num_builders; i++) {
				...
				ptr->Reset_Skim_Gap ();
				ptr->Update_Rider_Penalties ();
				ptr->Update_Park_Penalties ();
...
void Plan_Processor::Stop_Processing (bool save_flows)
{
	...
	if (num_threads > 1) {
		...
		if (save_flows || exe->Rider_Flag () || exe->Park_Demand_Flag ()) {
			for (int i=0; i < num_builders; i++) {
				if (save_flows) path_builder [i]->Save_Flows ();
				if (exe->Rider_Flag ()) path_builder [i]->Save_Riders ();
				if (exe->Park_Demand_Flag ()) path_builder [i]->Save_Park_Demand ();
			}
		}
	} else {
