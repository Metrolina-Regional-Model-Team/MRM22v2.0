...
void PlanPrep::Plan_Processing::Read_Plans (int part)
{
	...
	} else if (exe->plan_skim_flag) {
		process_type = "Exporting";
	} else if (exe->park_list_flag || exe->route_list_flag) {
		process_type = "Constraining";
	} else {
	...
	while (plan_file->Read_Plan (*plan_ptr)) {
		...
		if (exe->Trip_Sort () == TRAVELER_SORT) {
			...
			while (trip_index <= trip_rec) {
				if (trip_index == trip_rec) {
					if (exe->new_plan_flag) {
						if (plan_ptr->Mode () == merge_ptr->Mode ()) {
			...
			while (trip_index <= trip_rec) {
				if (exe->new_plan_flag && trip_index < trip_rec) {
					if (exe->merge_mode_flag && merge_ptr->Mode () != plan_ptr->Mode ()) goto next;
			...
			if (exe->plan_skim_flag) {
				plan_skim_file->Put_Data (*plan_ptr);

				if (!exe->select_parking || exe->parking_range.In_Range (plan_skim_file->Parking ())) {
