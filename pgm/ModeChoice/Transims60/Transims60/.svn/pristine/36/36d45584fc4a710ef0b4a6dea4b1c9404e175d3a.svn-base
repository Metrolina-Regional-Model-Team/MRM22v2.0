...
public:
	...
	{
		size_t num = map <Key, Type>::size ();
		if (data.Add_Data (&num, sizeof (num))) {
			if (num == 0) return (true);
			typename map <Key, Type>::iterator itr;
			for (itr = map <Key, Type>::begin (); itr != map <Key, Type>::end (); itr++) {
				Key key = itr->first;
				if (!data.Add_Data (&key, sizeof (Key))) return (false);
				if (!data.Add_Data (&itr->second, sizeof (Type))) return (false);
			}
			return (true);
		}
		return (false);
	...
	bool UnPack (Data_Buffer &data)
	{
		...
		if (data.Get_Data (&num, sizeof (num))) {
			...
			map <Key, Type>::clear ();
			for (int i = 0; i < (int) num; i++) {
				if (!data.Get_Data (&key, sizeof (key))) return (false);
				if (!data.Get_Data (&rec, sizeof (rec))) return (false);
				map <Key, Type>::insert (pair <Key, Type> (key, rec));
			}
			return (true);
		}
		return (false);
	}
	bool Pack (Pack_File &file)
	{
		size_t num = map <Key, Type>::size ();
		if (file.Write (&num, sizeof (num))) {
			if (num == 0) return (true);
			...
			for (itr = map <Key, Type>::begin (); itr != map <Key, Type>::end (); itr++) {
				Key key = itr->first;
				if (!file.Write (&key, sizeof (Key))) return (false);
				if (!file.Write (&(itr->second), sizeof (Type))) return (false);
			}
			return (true);
		}
		return (false);
	}
	bool UnPack (Pack_File &file)
	{
		size_t num;
		if (file.Get_Data (&num, sizeof (num))) {
			Key key;
			Type rec;
			map <Key, Type>::clear ();
			for (int i = 0; i < (int) num; i++) {
				if (!file.Read (&key, sizeof (key))) return (false);
...
template <typename Key, typename Type>
class Hash_Map : public hash_map <Key, Type>
{
public:
	Hash_Map (void) {}
	...
	bool Pack (Data_Buffer &data)
	{
		size_t num = hash_map <Key, Type>::size ();
		if (data.Add_Data (&num, sizeof (num))) {
			if (num == 0) return (true);
			typename hash_map <Key, Type>::iterator itr;
			for (itr = hash_map <Key, Type>::begin (); itr != hash_map <Key, Type>::end (); itr++) {
				Key key = itr->first;
				if (!data.Add_Data (&key, sizeof (Key))) return (false);
				if (!data.Add_Data (&itr->second, sizeof (Type))) return (false);
			}
	...
	bool UnPack (Data_Buffer &data)
	{
		...
		if (data.Get_Data (&num, sizeof (num))) {
			Key key;
			Type rec;
			hash_map <Key, Type>::clear ();
			for (int i = 0; i < (int) num; i++) {
				if (!data.Get_Data (&key, sizeof (key))) return (false);
				if (!data.Get_Data (&rec, sizeof (rec))) return (false);
				hash_map <Key, Type>::insert (pair <Key, Type> (key, rec));
			}
			return (true);
		}
		return (false);
