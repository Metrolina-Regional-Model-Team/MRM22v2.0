...
void Converge_Service::Iteration_Setup (void)
{
	...
	potential = counters.select_records;
	total_percent = 1.0;

	if (counters.total_records > 0 && max_percent_flag) {
		percent_selected = ((double) counters.select_records / counters.total_records);
		total_percent = percent_selected;

		if (percent_selected > max_percent_select) {
			percent_selected = max_percent_select / percent_selected;
			if (counters.select_weight > 0) {
				percent_selected = percent_selected * counters.select_records / counters.select_weight;
			}
		} else {
