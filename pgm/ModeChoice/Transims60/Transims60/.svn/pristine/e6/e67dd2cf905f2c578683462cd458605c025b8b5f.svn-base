...
void Select_Service::Select_Service_Keys (int *keys)
{
	Control_Key control_keys [] = { //--- code, key, level, status, type, default, range, help ----
		...
		{ MAXIMUM_PERCENT_SELECTED, "MAXIMUM_PERCENT_SELECTED", LEVEL0, OPT_KEY, FLOAT_KEY, "100.0 percent", "0.01..100.0 percent", NO_HELP },
...
void Select_Service::Read_Select_Keys (void)
{
	...
	if (exe->Control_Key_Status (SELECT_TRANSIT_MODES)) {
		select_transit_modes = exe->Transit_Range_Key (SELECT_TRANSIT_MODES, select_transit);

		if (select_transit_modes && !dat->System_File_Flag (TRANSIT_ROUTE)) {
			exe->Error ("Transit Routes are required for Transit Mode Processing");
		}
	}
...
//---------------------------------------------------------
//	Select_Plan_Parking_Lots
//---------------------------------------------------------

bool Select_Service::Select_Plan_Parking_Lots (Plan_Data &plan)
{
	Plan_Leg_Itr leg_itr;

	for (leg_itr = plan.begin (); leg_itr != plan.end (); leg_itr++) {
		if (leg_itr->Type () == PARKING_ID) {
			if (parking_range.In_Range (leg_itr->ID ())) return (true);
		}
	}
	return (false);
}
...
bool Select_Service::Select_Plan_Route_Modes (Plan_Data &plan)
{
	...
	for (leg_itr = plan.begin (); leg_itr != plan.end (); leg_itr++) {
		if (leg_itr->Type () == ROUTE_ID) {
			map_itr = dat->line_map.find (leg_itr->ID ());

			if (map_itr != dat->line_map.end ()) {
				Line_Data *line_ptr = &dat->line_array [map_itr->second];
