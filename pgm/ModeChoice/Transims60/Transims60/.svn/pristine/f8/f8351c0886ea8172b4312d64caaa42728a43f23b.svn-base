...
void ModeChoice::Execute (void)
{
	...
	if (auto_occ_flag) {
		veh_type_buckets.assign (veh_type_array.size (), 0.5);
	}
	...
	for (iteration=1; iteration <= max_iter; iteration++) {
		...
		//---- update the ridership penalties ----

		if (rider_flag) {
			line_array.Update_Penalties ();
		}

		//---- update the parking penalties ----

		if (penalty_update_flag) {
			park_demand_array.Update_Penalties (false);
		}
		if (save_flag) break;
