...
public:
	...
	void  From_Zone (int value)     { from_zone = (short) MAX (value, 0); }
	void  To_Zone (int value)       { to_zone = (short) MAX (value, 0); }
	void  From_Mode (int value)     { from_mode = (char) MAX (value, 0); }
	void  To_Mode (int value)       { to_mode = (char) MAX (value, 0); }
	void  Period (int value)        { period = (char) MAX (value, 0); }
	void  Class (int value)         { type = (char) MAX (value, 0); }
...
public:
	...
	string Period_String (int num)        {	return (fare_periods.Range_Format (num));	}
	int    Num_Periods (void)             { return (fare_periods.Num_Periods ()); }
