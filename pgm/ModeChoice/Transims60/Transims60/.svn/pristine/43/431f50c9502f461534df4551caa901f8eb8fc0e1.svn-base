...
void Path_Builder::Save_Riders (void)
{
	if (exe->Rider_Flag () && exe->Num_Threads () > 1) {
		MAIN_LOCK
		int l, s, r;
		...
		for (l = 0, line_itr = line_array.begin (); line_itr != line_array.end (); line_itr++, l++) {
...
//---------------------------------------------------------
//	Update_Rider_Penalties
//---------------------------------------------------------

void Path_Builder::Update_Rider_Penalties (void)
{
	if (exe->Rider_Flag () && exe->Num_Threads () > 1) {
		MAIN_LOCK
		int l, s, r;
		...
		for (l = 0, line_itr = exe->line_array.begin (); line_itr != exe->line_array.end (); line_itr++, l++) {
			line_ptr = &line_array [l];
			...
			for (s = 0, stop_itr = line_itr->begin (); stop_itr != line_itr->end (); stop_itr++, s++) {
				...
				for (r = 0, run_itr = stop_itr->begin (); run_itr != stop_itr->end (); run_itr++, r++) {
					...
					run_ptr->Penalty (run_itr->Penalty ());
					run_ptr->Board (0);
					run_ptr->Alight (0);
					run_ptr->Load (0);
					run_ptr->Factor (0);
...
void Path_Builder::Save_Park_Demand (void)
{
	if (exe->Park_Demand_Flag () && exe->Num_Threads () > 1) {
		exe->park_demand_array.Add_Demand (park_demand_array, true);
	}
}

//---------------------------------------------------------
//	Update_Park_Penalties
//---------------------------------------------------------

void Path_Builder::Update_Park_Penalties (void)
{
	if (exe->Park_Demand_Flag () && exe->Num_Threads () > 1) {
		park_demand_array.Copy_Penalty_Data (exe->park_demand_array, true);
	}
}
...
void Path_Builder::Initialize (Router_Service *_exe)
{
	...
	if (exe->Rider_Flag ()) {
		if (exe->Num_Threads () < 2) {
			line_array_ptr = &exe->line_array;
		} else {
#ifdef THREADS
			line_array_ptr = &line_array;
			line_array = exe->line_array;
#endif
		}
	}

	if (exe->Park_Demand_Flag ()) {
		if (exe->Num_Threads () < 2) {
			park_demand_array_ptr = &exe->park_demand_array;
		} else {
#ifdef THREADS
			park_demand_array_ptr = &park_demand_array;
			park_demand_array.Replicate (exe->park_demand_array);
#endif
		}
	}
