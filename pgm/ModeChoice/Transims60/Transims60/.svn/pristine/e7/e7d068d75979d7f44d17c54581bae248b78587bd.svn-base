...
void TransitNet::Execute (void)
{
	...
	} else if (new_transit_net_flag && equiv_flag) {
		Update_Fare_Zones ();
	}

	//---- read stop fare zone data ----
	...
	if (new_access_flag && (station_distance > 0 || (parkride_flag && PNR_distance > 0) || zone_stop_flag)) {
		Link_Data *link_ptr;
		Points points;
		XYZ_Point pt;
		Stop_Itr stop_itr;
	...
	if (new_access_flag) {
		if (!parkride_flag && !access_flag && transit_net_flag) {
			Parking_Access ();
		}
		if (zone_stop_flag) {
			Zone_Access ();
