Macro "Tour_TOD2_AMPeak_TRMG2" (Args)
...
//
	...
	HBU =  openmatrix(Dir + "\\ModeSplit\\HBU_PEAK_MS.mtx", "True")
        SCH =  openmatrix(Dir + "\\TripTables\\SCH_AMPEAK_TRIPS.mtx", "True")
	IE = openmatrix(Dir + "\\TripTables\\IE_AMPEAK_TRIPS.mtx", "True")
	EI = openmatrix(Dir + "\\TripTables\\EI_AMPEAK_TRIPS.mtx", "True")

	AMOD = openmatrix(Dir + "\\tod2\\ODHwyVeh_AMPeak.mtx", "True")
	...
	amod1  = CreateMatrixCurrency(AMOD, "SOV", "Rows", "Columns",)
	amod2  = CreateMatrixCurrency(AMOD, "Pool2", "Rows", "Columns",)
	amod3  = CreateMatrixCurrency(AMOD, "Pool3", "Rows", "Columns",)
...
//For Tour model, PAs and APs are combined for all purposes.  Only HBW, HBU, HBO & NHB need to be split from Peak to AM (& PM in the PM TOD2 macro).

//Drive alone
        MatrixOperations(amod1, {dahbw,dahbo,danhb,dasch,daei,daie,dahbu}, {0.4849,0.4397,0.4202,1.0,1.0,1.0,0.5232},,, {{"Operation", "Add"}, {"Force Missing", "No"}})

//Pool 2
        MatrixOperations(amod2, {p2hbw,p2hbo,p2nhb,p2sch,p2ei,p2ie,p2hbu}, {0.2425,0.2199,0.2102,1.0,1.0,1.0,0.2616},,, {{"Operation", "Add"}, {"Force Missing", "No"}})

//Pool 3+
        MatrixOperations(amod3, {p3hbw,p3hbo,p3nhb,p3sch,p3ei,p3ie,p3hbu}, {0.1362,0.1231,0.1148,1.0,1.0,1.0,0.1558},,, {{"Operation", "Add"}, {"Force Missing", "No"}})
...
//________OD tables:  Add EE vehicles and COM/TRUCK cores
	...
	odhtk = CreateMatrixCurrency(OD, "HTK", "Rows", "Columns",)
        ...
        MatrixOperations(odcom, {c1,c2,c3,c4,tc}, {0.0455,0.077,0.077,0.091,0.0455},,, {{"Operation", "Add"}, {"Force Missing", "No"}})
        MatrixOperations(odmtk, {m1,m2,m3,m4,tm}, {0.0455,0.077,0.077,0.091,0.0455},,, {{"Operation", "Add"}, {"Force Missing", "No"}})
        MatrixOperations(odhtk, {h1,h2,h3,h4,th}, {0.0455,0.077,0.077,0.091,0.0455},,, {{"Operation", "Add"}, {"Force Missing", "No"}})
...
*/
   ...
   goto quit
    ...
    quit:
		on error, notfound default
   		datentime = GetDateandTime()
		AppendToLogFile(1, "Exit Tour TOD2_AMPEAK " + datentime)
    	return({1, msg})
