...
//  DBox to select partial run of MRM	
	...
	Init do
		...
		modstatsjobs = 	{"Tour_RunStats","MSMatrixStats", "ODMatrixStats", "AvgTripLenTrips_tour", "VMTAQ", "HighwayCalibrationStats_tour"}
...
//12/1/16, mk; changed from above, wasn't allowing IS feedback to be run singly
	...
	Frame "partialmodelframe" 8, 1, 58, 6.0 Prompt: "Run Metrolina Tour Model starting with program" 

	//********************************************************
	
	Popdown Menu "startjob" 12, 2, 30 List: jobslist Variable: pick_jobindex do
...
//	Text " " same, after, , 0.5
	...
	//***************************************************************************************************************
	Frame 1, 6, 72, 34.5 prompt: "Select individual jobs " 
	Text "Run selections" 3.0, 8.0, 26
...
//		ShowArray(JobsToRun)
	...
	//***************************************************************************************************************
	// Tab list of jobs
	Tab List 2,10,70,31 variable: tab_index
...
//	Frame 1, 0.5, 67.5, 2.5
	Frame 1, 0.5, 71.0, 2.5
	Text "Select All" 2, 1.0, 26
...
/*	Text " " same, after, , 0.25
	...
	Checkbox " " after, same Variable: jobstatus.VMTAQ.runsel Help: "Create VMT / VHT tables for air quality reporting" do
	enditem

	Text " " same, after, , 0.25
	Text "Tour Run stats" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.Tour_RunStats.runsel Help: "All Tour specific run stats" do
	enditem

	Text " " same, after, , 0.25
	Text "Highway Calibration stats" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.HighwayCalibrationStats_tour.runsel Help: "Build count to assigned volumes comparison sets (screenline, atfun, ...)" do
	enditem
*/
	// column 2
...
Added Separate Model Statistics Tab
*/
	...
	//********************************************************
	Tab prompt: "Model Statistics"
	
//	Frame 1, 0.5, 67.5, 2.5
	Frame 1, 0.5, 71.0, 2.5
	Text "Select All" 2, 1.5, 26
	Checkbox "all_sel_flag5" after, same prompt: "" variable: all_sel5 do
		if (all_sel5 = 1) 
			then do
				for i = 1 to modstatsjobs.length do
					job = modstatsjobs[i]
					jobstatus.(job).runsel = 1
				end
/*				for i = 1 to runtranassnjobs.length do
...
*/				none_sel5 = 0
	...
	Text "Select None" 38.0, same, 26 
	Checkbox "none_sel_flag5" after, same prompt: "" variable: none_sel5 do
		if (none_sel5 = 1) 
			then do
				for i = 1 to modstatsjobs.length do
					job = modstatsjobs[i]
...
*/				all_sel5 = 0
	...
	// column 1
	Text "MODEL RUNSTATS" 2.0, 3.5

	Text " " same, after, , 1.0
	Text "Tour Run stats" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.Tour_RunStats.runsel Help: "Create all Tour specific run stats" do
	enditem
	
	Text " " same, after, , 0.25
	Text "Mode Choice Stats" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.MSMatrixStats.runsel Help: "Create Mode Choice run stats" do
	enditem

	Text " " same, after, , 0.25
	Text "OD Stats" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.ODMatrixStats.runsel Help: "Create OD run stats" do
	enditem
	Text " " same, after, , 0.25

	Text "Avg Trip Length" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.AvgTripLenTrips_tour.runsel Help: "Calculate Average Trip Lengths" do
	enditem
	...
	Text " " same, after, , 0.25
	Text "Transit Pax Stats" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.Transit_Pax_Stats.runsel Help: "Create transit \\RouteOut table" do
	enditem

	Text " " same, after, , 0.25
	Text "Transit Op Stats" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.Transit_Operations_Stats.runsel Help: "Create transit operations statistics tables" do
	enditem

	Text " " same, after, , 0.25
	Text "Transit Boardings" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.Transit_Boardings.runsel Help: "Route Ons/Offs by Stop (routes.dbf GRP_LIST = 1-4)" do
	enditem

	Text " " same, after, , 0.25
	Text "Transit RunStats" 2.0, after, 26
	Checkbox " " after, same Variable: jobstatus.Transit_RunStats.runsel Help: "Create download table for RunStats spreadsheet" do
	enditem
