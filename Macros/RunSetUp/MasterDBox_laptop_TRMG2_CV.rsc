Dbox "MasterDBox" center, top  Title: "Metrolina Regional Model: MRM22v1.0 (May 2022)"
	Init do
...
//********************* Start of Directory / File section *************************************************
	...
	//Tour Model
		runbasejobs_tour = {"Build_Networks", "Area_Type", "CapSpd", "RouteSystemSetUp", 
				...
				"TD_TranPath_Peak", "TD_TranPath_Free", 
				"Tour_DestinationChoice", "Tour_IS", "Tour_IS_Location", "Tour_TruckTGTD_TRMG2", 
				"Tour_ToD1", "Tour_TripAccumulator", "MS_RunPeak",
				"Tour_TOD2_AMPeak_TRMG2", "HwyAssn_RunAMPeak"}
		...
		runpostfeedbackjobs_tour = {"MS_RunOffPeak", "MSMatrixStats", //"Tour_TF_CountySummaryStats", 
				"Tour_TOD2_PMPeak_TRMG2", "Tour_TOD2_Midday_TRMG2", "Tour_TOD2_Night_TRMG2", 
				"HwyAssn_RunPMPeak", "HwyAssn_RunMidday", "HwyAssn_RunNight"}
//"TDMatrixStats", 
...
*/
	...
	Button " Create Tour Dir " 26.5, after, 16 Help: "Create a new run directory for tour based model " do
		modeltype = "Tour"
		CreateDirectory = 1
		RunMacro("GetDir")
		if DirExist = 1
...
//	Land Use (SE) file - stays disabled if run hasn't started previously - Land Use file is filled in DirFiles
	...
	//*****************************************************************************
	// Start model 
	Frame "modelframe" 1, 22.5, 65, 6.5 Prompt: "Run MRM Models" 
//	Frame "modelframe" 1, 22.5 Prompt: "Run MRM Models" 
	
	//Trips and Tour Model buttons occupy same space - enabled or disabled based on Model type
	
/*	Button " Run Trips Model " 3, 23.5, 30  Help: "Run Full TRIPS feedback model with directories and files above" do	
...
*/
	...
	Button " Run Tour Model " 3, 23.5, 30  Help: "Run Full TOUR feedback model with directories and files above" do	
...
/*	Button " Run Partial Trips Model " 3, 25.5, 30 Help: "Continue partial run, or run individual steps" do	
...
*/
	...
	Button " Run Partial Tour Model " 3, 25.5, 30 Help: "Continue partial run, or run individual steps" do	
		if keepgoing = "Yes" then RunMacro("SetArguments")
		if keepgoing = "No" 
			then do
				PlaySound(sysdir + "\\media\\Windows Critical Stop.wav", "null")
	...
	Text " " 3, 26.75, , 0.5
	Button " Hwy Assn - existing trip tables " 3, after, 30  Help: "HOT Highway assignment with existing trip tables" Disabled do	
		rtn = RunDBox("AssnonExistingTripTables", Args) 
	enditem

	Text " " same, after, , 0.5
	Button " Highway Select Link Analysis " 3, after, 30  Help: "Highway assignment - with select link enabled" Disabled do	
		ShowMessage("MRM Utilities -Highway Assign Select Link" + "\n" + "NEED TO ADD THIS JOB (old: HwyAssn_SelectLink)") 

	enditem

	Text " " same, after, , 0.5
	Button " MRM Utilities menu " 3, after, 30  Help: "Utilities" do
	...
	Text " " same, after, , 1.5
	Text " Messages " 36.0, 23.5
	Scroll List "messageslist" 36.0, 25, 62, 3.0 List: Message 

	Button " Show Args " 26.0, 34, 15 Help: "Show / modify Arguments file contents." do	
